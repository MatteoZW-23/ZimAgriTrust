...
export function TermsOfService({ onClose }) {
  return (
    <div className="modal-overlay">
      <div className="panel modal-container animate-rise">
        ...
        <div className="modal-body scrollable">
          ...
          <section className="legal-section">
            ...
            <p>Agents must not manipulate market data or trend recommendations for personal gain. Insights are derived from system-wide transaction history and MUST be presented to stakeholders without bias.</p>
          </section>
          <section className="legal-section">
            <h3>2. Zero Tolerance for Collusion</h3>
            <p>Any collusion between an agent and a market participant (Farmer/Buyer) to release escrow funds outside of verified delivery proof is grounds for immediate termination of platform authority.</p>
          </section>
          <section className="legal-section">
            <h3>3. Data Confidentiality & Encryption</h3>
            <p>Personal identifiers (National IDs, Phone Numbers, and Transactional Metadata) are encrypted and must only be accessed during active dispute resolution or verified onboarding workflows.</p>
          </section>
          <section className="legal-section">
            <h3>4. Escrow & Settlement Governance</h3>
            <p>Platform settlement depends on accurate data entry. Agents responsible for verifying quantity and grade must ensure physical alignment with the digital twin before confirming listings.</p>
          </section>
          <section className="legal-section" style={{ borderTop: '1.5px solid rgba(239, 68, 68, 0.1)', paddingTop: '20px', marginTop: '20px' }}>
            ...
            <ul className="legal-list" style={{ color: '#475569', fontSize: '13px', lineHeight: '1.6' }}>
                ...
                <li><strong>Escrow Interference:</strong> Unauthorized attempts to bypass the multisig settlement gateway or interfere with the automated handshake results in a Tier-1 security lockout.</li>
                <li><strong>Identity Mismatch:</strong> Any discrepancy between National ID registry data and biometric verification results in an immediate 'Unverified' freeze on all financial withdrawals.</li>
            </ul>
          </section>
        </div>
        <div className="modal-actions">
          <button className="q-btn primary-btn" onClick={onClose}>Close Legal View</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="panel modal-container animate-rise">
        ...
        <div className="modal-body scrollable">
          ...
          <ul className="legal-list">
            ...
            <li><strong>Transactional Data:</strong> All trade values and crop yields are anonymized for aggregate market trend analysis.</li>
            <li><strong>Data Retention:</strong> KYC data is retained for the duration of platform membership plus a mandatory 5-year audit trail.</li>
          </ul>
        </div>
        <div className="modal-actions">
          <button className="q-btn primary-btn" onClick={onClose}>Acknowledge Policy</button>
        </div>
      </div>
    </div>
  );
...
export function SecurityAuditLogs({ token, onClose }) {
  ...
  return (
    <div className="modal-overlay">
      <div className="panel modal-container animate-rise" style={{ maxWidth: "700px" }}>
        ...
        <div className="modal-actions">
          <button className="q-btn" onClick={onClose}>Exit Audit</button>
        </div>
      </div>
    </div>
  );
...
export function SettingsPanel({ profile, token, theme, setTheme, onSync }) {
  ...
  return (
    <div className={`v4-settings-layout ${isNavCollapsed ? 'nav-collapsed' : ''}`}>
      {activeModal === 'tos' && <TermsOfService onClose={() => setActiveModal(null)} />}
      ...
      <aside className="settings-sidebar-v4">
        <div className="sidebar-header">
            <h3>Settings</h3>
            <button className="nav-toggle-v4" onClick={() => setIsNavCollapsed(!isNavCollapsed)}>
                <i className={`fas ${isNavCollapsed ? 'fa-indent' : 'fa-outdent'}`}></i>
            </button>
        </div>
        <div className="settings-nav-v4">
            {tabs.map((tab) => (
            <button
                key={tab.id}
            ...
            >
                <span className="tab-icon"><i className={`fas ${tab.icon}`}></i></span>
                {!isNavCollapsed && <span className="tab-label">{tab.label}</span>}
                {activeTab === tab.id && !isNavCollapsed && <i className="fas fa-chevron-right ml-auto"></i>}
            </button>
            ))}
        </div>
        {!isNavCollapsed && (
            <div className="sidebar-footer-v4">
                <div className="sys-ver">Version 1.2.94-PRIME</div>
            </div>
        )}
      </aside>

      <div className="settings-workspace-v4">
        <div className="workspace-card panel card-glass animate-fade">
            {activeTab === "account" && (
            <div className="settings-page-content animate-fade">
                <h2 className="page-heading">My Account</h2>
                <div className="profile-hero-v4">
                    <div className="avatar-v4">{profile.full_name?.charAt(0)}</div>
                    <div className="hero-text">
                        <h3>{profile.full_name}</h3>
                        <div className="badge-stack">
                            <span className="v4-badge role">{profile.role}</span>
                            <span className="v4-badge trust">TRUST SCORE: {Math.round(profile.risk_score)}%</span>
                        </div>
                    </div>
                </div>
            ...
            {activeTab === "security" && (
            <div className="settings-page-content animate-fade">
                <h2 className="page-heading">Security</h2>
                <div className="settings-list-v4">
                    <CardRow label="Two-Factor Authentication" description="Require a secure OTP for every administrative login.">
                        <div className="v4-switch on"></div>
                    </CardRow>
                    <CardRow label="Active Marketplace Sessions" description="List of all devices currently authorized for this account.">
                        <button className="q-btn ghost small" onClick={() => setActiveModal('audit')}>View Security Audit</button>
                    </CardRow>
                    <CardRow label="Escrow Authority PIN" description="Separate security layer for high-value fund release.">
                        <button className="q-btn primary-btn small">Reset PIN</button>
                    </CardRow>
                </div>
            </div>
            )}
            ...
            <div className="settings-page-content animate-fade">
                <h2 className="page-heading">Display & Language</h2>
                <div className="settings-list-v4">
                    <CardRow label="Color Mode" description="Choose a visual mode or sync with your device settings.">
                        <div className="style-toggles">
                            <button className={`style-btn ${theme === 'light' ? 'active' : ''}`} onClick={() => setTheme && setTheme('light')}><i className="fas fa-sun"></i> Light</button>
                            <button className={`style-btn ${theme === 'dark' ? 'active' : ''}`} onClick={() => setTheme && setTheme('dark')}><i className="fas fa-moon"></i> Dark</button>
                            <button className={`style-btn ${theme === 'auto' ? 'active' : ''}`} onClick={() => setTheme && setTheme('auto')}><i className="fas fa-laptop"></i> Auto</button>
                        </div>
                    </CardRow>
                    ...
                    <CardRow label="Interface Visual Theme" description="Optimize for local field conditions or global office view.">
                        <div className="style-toggles">
                            <button className={`style-btn ${profile.agriStyle === 'zimbabwe' ? 'active' : ''}`} onClick={() => profile.onStyleChange('zimbabwe')}>Harare (ZIM)</button>
                            <button className={`style-btn ${profile.agriStyle === 'world' ? 'active' : ''}`} onClick={() => profile.onStyleChange('world')}>London (INT)</button>
                        </div>
                    </CardRow>
                </div>
            </div>
            ...
            {activeTab === "payments" && (
                <div className="settings-page-content animate-fade">
                    ...
                    <div className="settings-list-v4">
                        <CardRow label="Default Payout Channel" description="Primary gateway for all system reimbursements.">
                            <select className="v4-input select-field">
                                <option selected>EcoCash Wallet (Mobile)</option>
                                <option>Innbuck Disbursement (Zim)</option>
                                <option>Bank SWIFT (NMB)</option>
                            </select>
                        </CardRow>
                        <div className="financial-status mt-24">
                            <div className="balance-box">
                                <span className="l">Active Payout Account</span>
                                <span className="v">+263 77 *** 5678 (Primary)</span>
                            </div>
                            <button className="add-method-v4">+ Link Governance Account</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {activeTab === "verification" && (
                <div className="settings-page-content animate-fade">
                    <h2 className="page-heading">Trust Score</h2>
                    <div className="trust-v4-card">
                        <div className="gauge-box">
                            <svg viewBox="0 0 100 100">
                                <circle className="bg" cx="50" cy="50" r="45" />
                                <circle className="val" cx="50" cy="50" r="45" style={{ strokeDasharray: `${(profile.risk_score || 98) * 2.83} 283` }} />
                            </svg>
                            <div className="gauge-text">{Math.round(profile.risk_score || 98)}%</div>
                        </div>
                        <div className="trust-details">
                            <h3>Verified Authority</h3>
                            <p>Tier-1 system credentials confirmed by central registry.</p>
                            <div className="points-list">
                                <span><i className="fas fa-check-circle"></i> National ID Verified</span>
                                <span><i className="fas fa-check-circle"></i> Biometric Auth Active</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {activeTab === "governance" && (
                <div className="settings-page-content animate-fade">
                    <h2 className="page-heading">System Rules</h2>
                    <div className="governance-grid">
                        <div className="gov-card">
                            <h3><i className="fas fa-tower-broadcast"></i> Market Data & Sync</h3>
                            <div className="gov-controls">
                                <CardRow label="Price Checker" description="How often we check prices in other markets.">
                                    <select className="v4-input select-field">
                                ...
                                <CardRow label="Price Warning" description="Warn me if a price looks too high or too low.">
                                    <div className="v4-input-range-wrap">
                                        <input type="range" min="5" max="50" defaultValue="15" className="v4-range" />
                        ...
                        <div className="gov-card mt-24">
                            <h3><i className="fas fa-shield-halved"></i> Global Security Enclave</h3>
                            <div className="gov-controls">
                                <CardRow label="Platform Trust Threshold" description="Minimum Trust Score required for Escrow release.">
                                    <input type="number" className="v4-input text-field" defaultValue="70" />
                                </CardRow>
                                <CardRow label="System Integrity Sync" description="Force a global database and USSD gateway refresh.">
                                    <button className="q-btn primary-btn small w-full" onClick={onSync}>FORCE SYNC PLATFORM</button>
                                </CardRow>
                                <CardRow label="Administrative Audit" description="Review all critical governance actions and state changes.">
                                    <button className="q-btn ghost small w-full" onClick={() => setActiveModal('audit')}>Open History Enclave</button>
                                </CardRow>
                            </div>
                        </div>
                    </div>
            ...
            {activeTab === "help" && (
                <div className="settings-page-content animate-fade">
                    ...
                    <div className="help-grid-v4">
                         <div className="help-tile-v4" onClick={() => setActiveModal('tos')}>
                            <div className="tile-icon"><i className="fas fa-gavel"></i></div>
                            <div className="tile-info">
                                <h4>Operational Terms</h4>
                                <p>Review legal conduct protocols.</p>
                            </div>
                         </div>
                         <div className="help-tile-v4" onClick={() => setActiveModal('privacy')}>
                            <div className="tile-icon"><i className="fas fa-user-lock"></i></div>
                            <div className="tile-info">
                                <h4>Data Governance</h4>
                                <p>Platform privacy standards.</p>
                            </div>
                         </div>
                    </div>
                    
                    <div className="emergency-zone mt-48">
                        <h3 className="danger-text">System Resilience</h3>
