  ...
  return (
    <div className="v4-liquidity-banner-hardened">
        <div className="banner-context">
            <span style={{ fontSize: '9px', fontWeight: 800, color: 'var(--v4-text-dim)' }}>SYSTEM_NODE: ZW-HRE-01 // <span style={{ color: '#20963D' }}>ONLINE</span></span>
        </div>
        <div className="banner-metrics">
            <div className="metric-node"><label>LIQUIDITY_POOL</label><strong>$1,482,000.00</strong></div>
            <div className="divider-v"></div>
            <div className="metric-node"><label>VERIFIED_AGR_LAND</label><strong>42,105 HA</strong></div>
            <div className="divider-v"></div>
            <div className="metric-node"><label>ESCROW_UTILIZATION</label><strong>89.42%</strong></div>
            <div className="divider-v"></div>
            <div className="metric-node"><label>NETWORK_LATENCY</label><strong>0.04ms</strong></div>
        </div>
        <div className="banner-end">
            <span style={{ fontSize: '9px', fontWeight: 800, color: 'var(--v4-text-dim)', fontFamily: 'monospace' }}>LOAD: 14% // TEMP: OPTIMAL</span>
        </div>
    </div>
  );
});
...
function App() {
  ...
  const getGreeting = () => {
    ...
    const statusExtra = [
        ...
        "Your community is growing today.",
        "Ready to make an impact?",
        "Supporting Zimbabwe's farmers.",
        "Connecting the heart of agriculture."
    ];
    const randomExtra = statusExtra[Math.floor(Math.random() * statusExtra.length)];
  ...
  const [notifications, setNotifications] = useState([
    { id: 101, type: 'success', title: 'Listing Authorized', desc: 'Your Grade A Maize listing is now live across the national network.', channel: 'System', time: '10m ago', read: false },
    { id: 102, type: 'info', title: 'New Offer Received', desc: 'Leafy Greens Ltd submitted a bid for your Organic Tomatoes.', channel: 'SMS', time: '1h ago', read: false },
    { id: 103, type: 'warning', title: 'Payment Pending', desc: 'Escrow confirmation required for Order #ORD-882.', channel: 'System', time: '2h ago', read: true }
  ]);
    ...
    try {
      ...
      if (err.status === 401 || err.message?.includes("401")) {
        if (token === 'test-token-bypass') {
          console.warn("Bypass token active - ignoring sync failure for testing.");
          return;
        }
  ...
  if (!token && !isGuestMode) {
    const bypassLogin = (role, name) => {
      handleLogin('test-token-bypass', { 
        full_name: `${name} (Bypass)`, 
        role: role,
        id: 'bypass-id'
      });
    };
    ...
    return (
      <>
        <div style={{
          ...
          <div style={{ fontSize: '10px', fontWeight: 'bold', color: '#000E2B', textAlign: 'center' }}>TEST BYPASS</div>
          <button onClick={() => bypassLogin('ADMIN', 'Admin User')} style={{ padding: '8px', background: '#dc2626', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold' }}>Login as Admin</button>
          <button onClick={() => bypassLogin('AGENT', 'Agent User')} style={{ padding: '8px', background: '#2563eb', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold' }}>Login as Agent</button>
          <button onClick={() => bypassLogin('FARMER', 'Farmer User')} style={{ padding: '8px', background: '#16a34a', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold' }}>Login as Farmer</button>
          <button onClick={() => bypassLogin('BUYER', 'Buyer User')} style={{ padding: '8px', background: '#9333ea', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold' }}>Login as Buyer</button>
        </div>
    ...
    <main className={`shell-v4 theme-${role.toLowerCase()} ${isSidebarCollapsed ? 'sidebar-hidden' : ''}`}>
        ...
        <div className="sidebar-logo-v4">
             ...
             {!isSidebarCollapsed && (
                <div className="logo-text-v4">
                    ...
                    {token === 'test-token-bypass' && <span style={{ fontSize: '8px', color: '#ff4d4d', fontWeight: 900, background: 'rgba(255, 77, 77, 0.1)', padding: '2px 4px', borderRadius: '4px', marginTop: '4px', display: 'block' }}>DEVELOPER PREVIEW</span>}
                </div>
             )}
      ...
      <section className="main-wrapper-v4">
         <header className="v4-header">
              ...
              <div className="v4-search-bar-container">
                  <div className="v4-search-wrapper">
                      ...
                      <input type="text" placeholder="Search operational resources..." />
                  </div>
              </div>

              <div className="v4-telemetry-pill">
                  <i className="fas fa-tower-broadcast"></i>
                  <div className="t-meta">
                      <label>HARARE SYNC:</label>
                      <strong>12ms</strong>
                  </div>
              </div>

              <div className="h-right-group">
                   <button className="v4-icon-btn" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} title="Toggle Protocol">
                       <i className={`fas ${theme === 'dark' ? 'fa-sun' : 'fa-moon'}`}></i>
                   </button>
                   
                   <button className="v4-action-pill-btn" onClick={() => alert('Generating Encrypted Data Archive...')}>
        ...
        <div className="main-content-v4 overflow-auto">
            ...
            {/* ADMIN VIEWS */}
            {currentView === "users" && <UserDirectoryPanel users={users} profile={profile} token={token} onGovernance={handleGovernance} onRefresh={handleSync} />}
            {currentView === "marketplace" && (
                <VerificationPanel 
                    listings={reviewQueue} 
                    onVerify={async (id) => {
                        try {
            ...
            {currentView === "market-monitor" && <MarketplacePanel activities={marketActivities} loading={loading} />}
            ...
            {currentView === "logs" && <ActivityHistoryPanel activities={[
                { id: 1, type: 'DEPOSIT', partner: 'GRAIN MILLERS LTD', amount: 15400, status: 'COMPLETED' },
                { id: 2, type: 'SALE', partner: 'T. MILLER', amount: 6200, status: 'PENDING' },
                { id: 3, type: 'SYSTEM_SYNC', partner: 'HQ SERVER', amount: 0, status: 'COMPLETED' },
                { id: 4, type: 'DISPUTE', partner: 'FARMER B. PETERSON', amount: 1200, status: 'PENDING' }
            ]} />}
          ...
          <div className="modal-overlay" onClick={() => setShowSysInfo(false)}>
              <div className="v4-glass-card-premium animate-fade-in" style={{ maxWidth: '600px', background: '#020617', color: '#fff', border: '1.5px solid #1e293b', padding: '0', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
                  ...
                  <div style={{ padding: '24px', maxHeight: '400px', overflowY: 'auto', fontFamily: 'monospace', fontSize: '12px' }}>
                      {[
                        { n: 'Harare Hub', s: 'ONLINE', p: 'Fast', l: 'Good' },
                        { n: 'Bulawayo Node', s: 'ONLINE', p: 'Fast', l: 'Good' },
                        { n: 'Mutare Link', s: 'ONLINE', p: 'Stable', l: 'Normal' },
                        { n: 'Gweru Gateway', s: 'SYNCING', p: '---', l: '---' },
                        { n: 'Masvingo Site', s: 'OFFLINE', p: '---', l: '---' },
                      ].map(node => (
      ...
      <footer className="v4-tech-status-bar" onClick={() => setShowSysInfo(true)} style={{ cursor: 'pointer', pointerEvents: 'auto' }}>
          <div className="status-item"><span className="dot pulse"></span> ALL SYSTEMS READY</div>
          <div className="divider-v"></div>
          <div className="status-item">CONNECTION: SECURE</div>
          <div className="divider-v"></div>
          <div className="status-item">ENCRYPTION: AES-256-GCM</div>
          <div className="divider-v"></div>
          <div className="status-item">NODE: ZW-HRE-01</div>
          <div className="ml-auto"></div>
          <div className="status-item opacity-50">v4.3.1-STABLE // BUILD_ID: 7f8e9a</div>
      </footer>
    </main>
  );
}
