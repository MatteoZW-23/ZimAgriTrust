...
import { confirmDelivery, submitReview } from '../api';
import { theme } from '../styles';

export default function ConfirmationScreen({ navigation, route }) {
  const { orderId, token, role = 'buyer' } = route.params || {};
  ...
  const handleConfirm = async () => {
    if (!orderId) { Alert.alert('Error', 'Order ID missing.'); return; }
    setConfirming(true);
    try {
      await confirmDelivery(token, orderId, null);
      if (rating > 0) {
      ...
      Alert.alert('Payment Released', 'Delivery confirmed! Funds have been released to the seller.', [
        { text: 'OK', onPress: () => navigation.navigate('MyOrders', { token, role }) }
      ]);
    } catch (err) {
      Alert.alert('Error', err.message || 'Failed to confirm delivery. Please try again.');
    } finally {
      setConfirming(false);
    }
  };
  ...
  const handleDispute = () => {
    navigation.navigate('OrderDetails', { orderId, role, token });
  };
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.heroSection}>
        ...
        <Text style={styles.heroSub}>
          Please confirm that you have inspected the product. Once confirmed, funds will be released to the seller.
        </Text>
      </View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.card}>
          ...
          <TextInput
            style={styles.input}
            placeholder="Enter verified quality grade (e.g., A, B, C)"
            value={quality}
      ...
      <View style={styles.section}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.starRow}>
            ...
            <Text style={styles.starVal}>({rating}.0)</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  heroSection: { padding: 40, backgroundColor: '#F9F9F9', alignItems: 'center' },
  ...
  input: { height: 50, borderRadius: 12, backgroundColor: '#F9F9F9', paddingHorizontal: 16, fontSize: 14, borderWidth: 1, borderColor: '#EEE' },
