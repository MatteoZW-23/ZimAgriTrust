    ...
    <section className="marketplace-section" id="marketplace">
      <div className="container">
        <div className="section-header fade-in">
          ...
          <h2>Available Crops & Inputs</h2>
          <p>Browse what's available from farmers and suppliers across Zimbabwe.</p>
        </div>
        ...
        <div className="listings-grid">
          {loading ? (
            ...
            listings.map((item) => (
              <ListingCard key={item.id} item={item} onBuy={() => !isAuthenticated ? onOpenAuth() : console.log("Buy", item.id)} />
            ))
          ) : (
        ...
        <div className="market-footer fade-in">
          <div className="footer-promo">
            ...
            <p>List your produce and connect directly with buyers. Registration is free.</p>
...
function ListingCard({ item, onBuy }) {
  ...
  return (
    <div className={`listing-card ${item.boosted ? 'boosted' : ''} fade-in`}>
      <div className="card-header">
        <div className="image-placeholder">
          ...
          {verified && (
            <div className="verified-badge" title="Verified Listing">
      ...
      <div className="card-body">
        <div className="category-tag">{isCrop ? 'Commodity' : 'Farm Input'}</div>
        ...
        <button className="btn btn-outline btn-block buy-btn" onClick={onBuy}>
          {isCrop ? 'View Details' : 'Purchase Input'} <ArrowRight size={16} />
        </button>
      </div>
    </div>
  );
