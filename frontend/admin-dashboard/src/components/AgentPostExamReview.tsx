...
export default function AgentPostExamReview({ token }) {
  ...
  return (
    <div className="v4-panel animate-fade-in" style={{ padding: 24 }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          <h2 style={{ fontSize: 24, fontWeight: 800, marginBottom: 4 }}>
            <i className="fas fa-user-shield"></i> Agent Post-Exam Review
          </h2>
          <p style={{ color: '#94a3b8', fontSize: 13 }}>
            Practical assessments • Shadowing • Supervised period • Promotions
          </p>
        </div>
        <button className="v4-btn v4-btn-secondary" onClick={loadAgents}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
        <select value={filter.status} onChange={(e) => setFilter((f) => ({ ...f, status: e.target.value }))}
                className="v4-select" style={selectStyle}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: 16 }}>
        {/* ── Agents list ───────────────────────────────────────────── */}
        <div style={{ background: '#0f172a', borderRadius: 12, padding: 12, maxHeight: '70vh', overflowY: 'auto' }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 8, color: '#94a3b8' }}>
          ...
          {agents.map((a) => (
            ...
            >
              <div style={{ fontWeight: 600, fontSize: 13 }}>{a.full_name || a.agent_code}</div>
              <div style={{ fontSize: 11, color: '#94a3b8' }}>
                {a.agent_code} • {a.specialization} • <strong>{a.status}</strong>
              </div>
        ...
        {/* ── Agent detail ────────────────────────────────────────── */}
        <div style={{ background: '#0f172a', borderRadius: 12, padding: 16, minHeight: '70vh' }}>
          {!selected && (
            <p style={{ color: '#64748b', textAlign: 'center', marginTop: 80 }}>
              Select an agent to review their post-exam progress.
            </p>
          )}
          ...
          {selected && (
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                <div>
                  <h3 style={{ fontSize: 20, fontWeight: 800 }}>{selected.full_name}</h3>
                  <div style={{ color: '#94a3b8', fontSize: 13 }}>
                    <code>{selected.agent_code}</code> • {selected.specialization} • Rating ⭐ {selected.rating}
                  </div>
                </div>
                <button className="v4-btn v4-btn-primary" onClick={() => handlePromote(selected.id)}>
                  <i className="fas fa-arrow-up"></i> Evaluate Promotion
                </button>
              </div>
              ...
              {detail && (
                <>
                  <Section title="Practical Assessment" icon="fa-clipboard-check">
                    <PracticalSummary data={detail.practical} />
                  </Section>

                  <Section title="Shadowing Period (10 tasks required)" icon="fa-eye">
                    <ProgressBar current={detail.shadowing.approved_count} total={detail.shadowing.required} />
                    <div style={{ marginTop: 8, maxHeight: 200, overflowY: 'auto' }}>
                      {detail.shadowing.logs?.length === 0 && (
                        <p style={{ color: '#64748b', fontSize: 12 }}>No shadow logs yet.</p>
                      )}
                      {detail.shadowing.logs?.slice(0, 10).map((log) => (
                        <div key={log.id} style={miniRow}>
                          <span style={{ ...statusDot, background: shadowColor(log.status) }} />
                          <span style={{ flex: 1, fontSize: 12 }}>
                            <strong style={{ textTransform: 'capitalize' }}>{log.task_type}</strong>{' '}
                            <span style={{ color: '#64748b' }}>
                              ({new Date(log.created_at).toLocaleDateString()})
                            </span>
                          </span>
                          <span style={{ fontSize: 11, color: shadowColor(log.status), fontWeight: 600 }}>
                            {log.status}
                          </span>
                        </div>
                      ))}
                  ...
                  <Section title="Supervised Period (20 tasks, 95% accuracy required)" icon="fa-user-graduate">
                    <ProgressBar current={detail.supervised.approved_count} total={detail.supervised.required} />
                    <div style={{ display: 'flex', gap: 16, marginTop: 8, fontSize: 13 }}>
                      <span>Average Accuracy:{' '}
                        <strong style={{ color: detail.supervised.average_accuracy >= 95 ? '#16a34a' : '#f59e0b' }}>
                          {detail.supervised.average_accuracy}%
                        </strong>
                      </span>
                      <span style={{ color: '#64748b' }}>
                        Required: {detail.supervised.min_accuracy_required}%
                      </span>
                    </div>
                  </Section>
                </>
...
function Section({ title, icon, children }) {
  return (
    <div style={{ background: '#1e293b', padding: 14, borderRadius: 10, marginBottom: 12 }}>
      <h4 style={{ fontSize: 14, fontWeight: 700, marginBottom: 10 }}>
        <i className={`fas ${icon}`} style={{ marginRight: 8, color: '#16a34a' }}></i>
        {title}
      </h4>
      {children}
    </div>
...
function PracticalSummary({ data }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8 }}>
      {TESTS.map((t) => {
        ...
        return (
          <div key={t.id} style={{
            ...
            <div style={{ fontSize: 18, fontWeight: 700 }}>{r ? `${r.score}%` : '—'}</div>
            <div style={{ fontSize: 10 }}>
              {r ? (r.passed ? '✅ Passed' : `❌ ≥ ${r.passing_score}%`) : `Need ${t.required}%`}
            </div>
            {r?.attempts > 1 && (
      ...
      {data.all_passed && (
        <div style={{ gridColumn: '1 / -1', color: '#16a34a', fontWeight: 700, fontSize: 13, marginTop: 4 }}>
  ...
  return (
    <div>
      ...
      <div style={{ height: 6, background: 'rgba(255,255,255,0.08)', borderRadius: 3, overflow: 'hidden' }}>
        <div style={{ height: '100%', width: `${pct}%`, background: pct >= 100 ? '#16a34a' : '#3b82f6', transition: 'width 0.3s' }} />
      </div>
    </div>
  );
}
