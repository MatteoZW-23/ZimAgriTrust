...
export default function StaffLoginScreen({ onLogin }) {
  ...
  return (
    <div className="admin-login-page">
      ...
      <div className="admin-login-shell animate-fade-in">
        <section className="admin-login-hero">
          <div className="admin-brand-lockup">
            <div className="admin-logo-mark">
              <img src="/logo.png" alt="ZimAgriTrust" />
            </div>
            <div>
              <p>ZimAgriTrust</p>
          ...
          <div className="admin-hero-copy">
            <span className="admin-eyebrow"><i className="fas fa-shield-halved"></i> Secure staff access</span>
            <h1>Manage the national agriculture trust network.</h1>
            <p>Monitor verification, escrow, disputes, agents, logistics and marketplace integrity from one protected admin console.</p>
          </div>
          <div className="admin-login-metrics">
            <div><strong>2FA</strong><span>Protected</span></div>
            <div><strong>24/7</strong><span>Oversight</span></div>
            <div><strong>Audit</strong><span>Ready</span></div>
          </div>
        </section>

        <section className="admin-login-card">
          <div className="admin-card-header">
            <span className="admin-card-badge">{step === 1 ? 'Admin Login' : 'Identity Verification'}</span>
            <h2>{step === 1 ? 'Welcome back' : 'Enter security code'}</h2>
            <p>{step === 1 ? 'Sign in using your registered staff phone number and password.' : 'Enter the 6-digit code sent to your verified channel.'}</p>
          </div>

          {error && (
            <div className="admin-error-alert slide-down">
          ...
          {step === 1 ? (
            <form onSubmit={handleLoginSubmit} className="admin-login-form">
              <div className="admin-field">
                ...
                <div className="admin-input-group">
                  <span className="admin-prefix">+263</span>
                  <input 
                    type="tel" 
                    value={phoneNumber}
              ...
              <div className="admin-field">
                ...
                <div className="admin-input-group">
                   <input 
                      ...
                      required 
                   />
                   <button type="button" className="admin-icon-btn" onClick={() => setShowPin(v => !v)} tabIndex={-1}>
              ...
              <button type="submit" className="admin-submit-btn" disabled={loading}>
                {loading ? <i className="fas fa-spinner fa-spin"></i> : <><span>Access dashboard</span><i className="fas fa-arrow-right"></i></>}
              </button>
            </form>
          ) : (
            <form onSubmit={handleVerify2FA} className="admin-login-form">
               <div className="admin-field">
                 ...
                 <div className="admin-input-group otp-group">
                   <input 
                     type="text" 
               ...
               <button type="submit" className="admin-submit-btn" disabled={loading}>
                 {loading ? 'Verifying...' : <><span>Verify and continue</span><i className="fas fa-check"></i></>}
               </button>
               
               <button type="button" className="admin-secondary-btn" onClick={() => setStep(1)}>
          ...
          <div className="admin-card-footer">
            ...
            <span>Encrypted staff session. Unauthorized access is logged.</span>
          </div>
        </section>
      </div>
      ...
      {forgotStep > 0 && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
          <div style={{ background: '#0f172a', border: '1px solid rgba(59,130,246,0.3)', borderRadius: '24px', padding: '40px', width: '100%', maxWidth: '400px' }}>
            <h3 style={{ fontWeight: 900, color: '#fff', marginBottom: '8px' }}>🔑 Reset Password</h3>
            <p style={{ fontSize: '13px', color: '#94a3b8', marginBottom: '24px' }}>
              {forgotStep === 1 ? "Enter your registered phone number." : "Enter the WhatsApp code and set a new Password."}
            </p>
            ...
            {forgotStep === 1 ? (
              ...
              <form onSubmit={handleForgotReset}>
                ...
                <div style={{ display: 'flex', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', overflow: 'hidden', marginBottom: '16px', background: 'rgba(0,0,0,0.3)' }}>
                  <input type={showNewPin ? "text" : "password"} value={newPin} onChange={e => setNewPin(e.target.value)} placeholder="New Password" required style={{ flex: 1, border: 'none', padding: '12px 16px', fontSize: '20px', textAlign: 'center', letterSpacing: '0.1em', outline: 'none', background: 'transparent', color: '#fff' }} />
