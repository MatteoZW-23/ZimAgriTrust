...
export default function BroadcastScreen() {
  ...
  return (
    <div className="broadcast-screen" style={{ padding: '24px', color: 'var(--v4-text)' }}>
      <div style={{ marginBottom: '24px' }}>
        <h2 style={{ fontSize: '24px', fontWeight: 900, margin: 0 }}>📢 System Broadcast</h2>
        <p style={{ fontSize: '14px', color: '#64748b', marginTop: '4px' }}>Send urgent announcements to target user groups via multiple channels.</p>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '32px' }}>
        {/* Composition Form */}
        <div style={{ background: 'var(--v4-card)', borderRadius: '24px', padding: '32px', border: '1px solid var(--v4-border)' }}>
          <h3 style={{ margin: '0 0 24px 0', fontSize: '18px', fontWeight: 800 }}>Compose Message</h3>
          
          <form onSubmit={handleSubmit}>
            <div style={{ marginBottom: '20px' }}>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 900, color: '#64748b', marginBottom: '8px' }}>TARGET AUDIENCE</label>
              <select 
                value={form.audience} 
            ...
            <div style={{ marginBottom: '20px' }}>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 900, color: '#64748b', marginBottom: '8px' }}>DELIVERY CHANNELS</label>
              <div style={{ display: 'flex', gap: '12px' }}>
                {['whatsapp', 'sms', 'email'].map(ch => (
                  ...
                  >
                    <i className={`fas fa-${ch === 'whatsapp' ? 'whatsapp' : ch === 'sms' ? 'comment-alt' : 'envelope'} mr-2`}></i>
                    {ch}
                  </button>
                ))}
              </div>
            ...
            <div style={{ marginBottom: '20px' }}>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 900, color: '#64748b', marginBottom: '8px' }}>SUBJECT (FOR EMAIL)</label>
              <input 
                type="text" 
                value={form.subject}
            ...
            <div style={{ marginBottom: '24px' }}>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 900, color: '#64748b', marginBottom: '8px' }}>MESSAGE BODY</label>
              <textarea 
                value={form.message}
        ...
        {/* Recent History */}
        <div>
          <h3 style={{ margin: '0 0 24px 0', fontSize: '18px', fontWeight: 800 }}>Recent History</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {loading ? (
              <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>Loading history...</div>
            ) : history.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '40px', color: '#94a3b8', background: '#f8fafc', borderRadius: '24px' }}>No previous broadcasts found.</div>
            ) : history.map(item => (
              <div key={item.id} style={{ background: '#fff', borderRadius: '20px', padding: '20px', border: '1px solid var(--v4-border)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                  <span style={{ fontSize: '11px', fontWeight: 900, color: 'var(--v4-primary)', textTransform: 'uppercase' }}>{item.audience} AUDIENCE</span>
                  <span style={{ fontSize: '11px', color: '#94a3b8' }}>{new Date(item.created_at).toLocaleDateString()}</span>
                </div>
                <div style={{ fontWeight: 800, fontSize: '14px', marginBottom: '8px' }}>{item.subject || 'No Subject'}</div>
                <div style={{ fontSize: '13px', color: '#475569', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', marginBottom: '16px' }}>
                  {item.message}
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #f1f5f9', paddingTop: '12px' }}>
                  ...
                  <div style={{ fontSize: '11px', fontWeight: 800, color: '#10b981' }}>
                    Sent: {item.sent_count} | Deliv: {item.delivered_count}
                  </div>
                </div>
              </div>
            ))}
