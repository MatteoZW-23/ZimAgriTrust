...
import { DollarSign, Download, TrendingUp, Shield } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatCard icon={DollarSign} label="Total Withdrawn" value={`$${totals.total_withdrawn.toFixed(2)}`} color="green" />
        <StatCard icon={TrendingUp} label="Total Fees" value={`$${totals.total_fees.toFixed(2)}`} color="red" />
        <StatCard icon={DollarSign} label="Net Payouts" value={`$${totals.total_net.toFixed(2)}`} color="blue" />
      </div>
              ...
              <tbody>
                {payouts.map((payout) => (
                  <tr key={payout.id} className="border-b border-earth-50">
                    <td className="py-4 text-earth-800">{new Date(payout.created_at).toLocaleDateString()}</td>
                    <td className="py-4 text-earth-600 capitalize">{payout.method}</td>
                    <td className="py-4 text-right font-bold text-earth-800">${payout.amount.toFixed(2)}</td>
                    <td className="py-4 text-right text-earth-600">${payout.fee.toFixed(2)}</td>
                    <td className="py-4 text-right font-bold text-earth-800">${payout.net_amount.toFixed(2)}</td>
