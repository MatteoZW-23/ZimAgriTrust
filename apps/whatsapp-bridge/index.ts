...
client.on('message', async (msg: any) => {
    ...
    try {
        ...
        try {
            ...
            console.warn('Could not resolve WhatsApp contact:', contactError?.message || contactError);
        }
        ...
        console.error('Error forwarding message to backend:', error?.message || error);
...
const scheduleRestart = (delayMs = 5000) => {
    ...
    setTimeout(async () => {
        ...
        try {
            ...
            console.error('Re-initialization failed:', err?.message || err);
