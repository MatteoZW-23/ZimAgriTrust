...
const DOCUMENT_SLOTS = [
  { key: 'doc_national_id_front',    label: 'National ID – Front',   emoji: '🪪' },
  { key: 'doc_national_id_back',     label: 'National ID – Back',    emoji: '🔄' },
  { key: 'doc_license_front',        label: "Driver's License – Front", emoji: '📋' },
  { key: 'doc_license_back',         label: "Driver's License – Back",  emoji: '🔄' },
  { key: 'doc_vehicle_registration', label: 'Vehicle Registration',  emoji: '📄' },
  { key: 'doc_vehicle_photo',        label: 'Vehicle Photo',         emoji: '🚚' },
  { key: 'doc_profile_photo',        label: 'Profile Photo',         emoji: '🤳' },
  { key: 'doc_live_selfie',          label: 'Live Selfie',           emoji: '📸' },
];
  ...
  return (
    <div className="v4-dashboard-container animate-fade-in compact-mode">
      ...
      <header className="v4-hero-professional theme-agent">
        <div className="hero-content-v4">
          <div className="kicker">
            <span className="pill">DRIVER COMMAND</span>
            <div className="sync-pulse">
              <div className="p-dot"></div>
              FLEET MANAGEMENT ACTIVE
            </div>
          </div>
          <h1>Driver <span>Verification</span> Centre.</h1>
          <p>Review driver applications, verify documents, and manage the transport fleet. Approve or reject with a reason — drivers are notified instantly.</p>
        </div>
        <div className="hero-visual">
          <div className="v4-glass-card-mini">
            <label>PENDING APPLICATIONS</label>
            <strong>{tabCounts['PENDING_REVIEW'] || 0} Drivers</strong>
            <div className="v4-progress-bar">
              <div style={{ width: `${Math.min(100, ((tabCounts['PENDING_REVIEW'] || 0) / 20) * 100)}%` }}></div>
            </div>
          </div>
        </div>
      </header>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '8px', flexWrap: 'wrap' }}>
        {STATUS_TABS.map(t => (
          <div key={t.key} style={{ flex: 1, minWidth: '120px', padding: '16px', background: 'var(--v4-surface)', borderRadius: '14px', border: `2px solid ${tab === t.key ? t.color : 'var(--v4-border)'}`, cursor: 'pointer', transition: 'all 0.15s' }}
            ...
            <div style={{ fontSize: '22px', fontWeight: 950, color: t.color }}>{tabCounts[t.key] || 0}</div>
            <div style={{ fontSize: '11px', fontWeight: 800, color: 'var(--v4-text-dim)', marginTop: '4px' }}>{t.label}</div>
          </div>
        ))}
        <button onClick={load} style={{ padding: '16px 20px', borderRadius: '14px', border: '1.5px solid var(--v4-border)', background: 'transparent', cursor: 'pointer', color: 'var(--v4-text-dim)', fontWeight: 800, alignSelf: 'stretch' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: selected ? '1fr 440px' : '1fr', gap: '24px' }}>
        ...
        <div className="v4-glass-card-premium">
          {loading ? (
            <div style={{ padding: '80px', textAlign: 'center', color: 'var(--v4-text-dim)' }}>
              ...
              <p style={{ marginTop: '16px', fontWeight: 700 }}>Loading drivers...</p>
            </div>
          ) : filtered.length === 0 ? (
            <div style={{ padding: '80px', textAlign: 'center' }}>
              <div style={{ fontSize: '56px', marginBottom: '16px' }}>🚚</div>
              <h3 style={{ fontWeight: 900, margin: '0 0 8px' }}>
                {tab === 'PENDING_REVIEW' ? 'No pending applications' : `No ${tab.toLowerCase().replace('_', ' ')} drivers`}
              </h3>
              <p style={{ color: 'var(--v4-text-dim)', fontWeight: 600 }}>
                {tab === 'PENDING_REVIEW' ? 'All driver applications have been reviewed.' : 'Nothing here yet.'}
              </p>
            </div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: '0 10px' }}>
              <thead>
                <tr style={{ color: 'var(--v4-text-dim)', fontSize: '10px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                  <th style={{ textAlign: 'left', padding: '0 20px' }}>Driver</th>
                  <th style={{ textAlign: 'left', padding: '0 20px' }}>Vehicle</th>
                  <th style={{ textAlign: 'left', padding: '0 20px' }}>Documents</th>
                  <th style={{ textAlign: 'left', padding: '0 20px' }}>Applied</th>
                ...
                {filtered.map(d => {
                  ...
                  return (
                    <tr key={d.id}
                      ...
                      onClick={() => { setSelected(d); setRejectNote(''); }}>
                      <td style={{ padding: '18px 20px', borderRadius: '14px 0 0 14px', border: '1.5px solid var(--v4-border)', borderRight: 'none' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                          <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: '#f1f5f9', display: 'grid', placeItems: 'center', fontWeight: 900, fontSize: '16px', color: '#000E2B', flexShrink: 0 }}>
                            {d.name?.charAt(0) || '?'}
                          </div>
                          <div>
                            <div style={{ fontWeight: 800, fontSize: '14px' }}>{d.name}</div>
                            <div style={{ fontSize: '11px', color: 'var(--v4-text-dim)', fontWeight: 700 }}>{d.phone}</div>
                          </div>
                        </div>
                      </td>
                      <td style={{ padding: '18px 20px', borderTop: '1.5px solid var(--v4-border)', borderBottom: '1.5px solid var(--v4-border)' }}>
                        <div style={{ fontWeight: 800, fontSize: '13px' }}>{d.vehicle_reg}</div>
                        <div style={{ fontSize: '11px', color: 'var(--v4-text-dim)', fontWeight: 600 }}>{d.vehicle_type || 'Unknown type'}</div>
                      </td>
                      <td style={{ padding: '18px 20px', borderTop: '1.5px solid var(--v4-border)', borderBottom: '1.5px solid var(--v4-border)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <div style={{ width: '60px', height: '6px', background: '#f1f5f9', borderRadius: '3px', overflow: 'hidden' }}>
                            <div style={{ width: `${(docCount / DOCUMENT_SLOTS.length) * 100}%`, height: '100%', background: docCount === DOCUMENT_SLOTS.length ? '#22c55e' : docCount > 4 ? '#f59e0b' : '#ef4444', borderRadius: '3px' }} />
                          </div>
                          <span style={{ fontSize: '11px', fontWeight: 800, color: 'var(--v4-text-dim)' }}>{docCount}/{DOCUMENT_SLOTS.length}</span>
                        </div>
                      </td>
                      <td style={{ padding: '18px 20px', borderTop: '1.5px solid var(--v4-border)', borderBottom: '1.5px solid var(--v4-border)', fontSize: '12px', color: 'var(--v4-text-dim)', fontWeight: 700 }}>
                        {d.created_at ? new Date(d.created_at).toLocaleDateString('en-ZW', { day: 'numeric', month: 'short', year: 'numeric' }) : '—'}
                      </td>
                        ...
                        <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                          ...
                          {tab === 'SUSPENDED' && (
                            <button className="q-btn ghost small" style={{ color: '#22c55e' }}
                              onClick={e => { e.stopPropagation(); handleReactivate(d); }}>
        ...
        {/* Review panel */}
        {selected && (
          <div className="v4-glass-card-premium" style={{ padding: '28px', display: 'flex', flexDirection: 'column', gap: '20px', alignSelf: 'start', position: 'sticky', top: '24px', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h3 style={{ margin: 0, fontWeight: 900, fontSize: '16px' }}>Driver Review</h3>
            ...
            <div style={{ padding: '16px', background: 'var(--v4-surface)', borderRadius: '12px' }}>
              <div style={{ fontWeight: 900, fontSize: '16px', marginBottom: '4px' }}>{selected.name}</div>
              <div style={{ fontSize: '12px', color: 'var(--v4-text-dim)', fontWeight: 700 }}>{selected.phone}</div>
              <div style={{ marginTop: '12px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                <div style={{ padding: '10px', background: 'var(--v4-bg)', borderRadius: '8px' }}>
                  <div style={{ fontSize: '10px', color: 'var(--v4-text-dim)', fontWeight: 800, textTransform: 'uppercase' }}>Vehicle</div>
                  <div style={{ fontSize: '13px', fontWeight: 800, marginTop: '2px' }}>{selected.vehicle_reg}</div>
                </div>
                <div style={{ padding: '10px', background: 'var(--v4-bg)', borderRadius: '8px' }}>
                  <div style={{ fontSize: '10px', color: 'var(--v4-text-dim)', fontWeight: 800, textTransform: 'uppercase' }}>Type</div>
                  <div style={{ fontSize: '13px', fontWeight: 800, marginTop: '2px' }}>{selected.vehicle_type || '—'}</div>
                </div>
              </div>
            </div>
            ...
            {/* Documents */}
            <div>
              <div style={{ fontSize: '11px', fontWeight: 900, color: 'var(--v4-text-dim)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '12px' }}>
                Documents ({DOCUMENT_SLOTS.filter(s => selected[s.key]).length}/{DOCUMENT_SLOTS.length})
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {DOCUMENT_SLOTS.map(doc => {
                  ...
                  return (
                    <div key={doc.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: 'var(--v4-bg)', borderRadius: '10px', border: `1.5px solid ${hasDoc ? 'var(--v4-border)' : '#fecaca'}` }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <span style={{ fontSize: '16px' }}>{doc.emoji}</span>
                        <span style={{ fontWeight: 700, fontSize: '12px' }}>{doc.label}</span>
                      </div>
                      {hasDoc ? (
                        <button
                          onClick={() => setDocModal({ url, label: doc.label })}
                          style={{ padding: '5px 12px', borderRadius: '8px', background: '#000E2B', color: '#fff', border: 'none', fontWeight: 800, fontSize: '11px', cursor: 'pointer' }}>
                          View
                        </button>
                      ) : (
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {[
                ...
                <span key={f.label} style={{ padding: '4px 10px', borderRadius: '8px', fontSize: '11px', fontWeight: 800, background: f.ok ? '#dcfce7' : '#fef3c7', color: f.ok ? '#166534' : '#92400e' }}>
                  {f.ok ? '✓' : '⏳'} {f.label}
                </span>
              ))}
            </div>
            ...
            {tab === 'PENDING_REVIEW' && (
              <>
                ...
                <button onClick={handleApprove} disabled={acting}
                  style={{ width: '100%', padding: '14px', background: '#22c55e', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: 900, fontSize: '14px', cursor: 'pointer', opacity: acting ? 0.7 : 1 }}>
                  {acting ? 'Processing...' : '✅ Approve Driver & Notify'}
                </button>
                ...
                <div>
                  <label style={{ fontSize: '11px', fontWeight: 900, color: '#ef4444', textTransform: 'uppercase', letterSpacing: '0.1em', display: 'block', marginBottom: '8px' }}>
                    Rejection Reason <span style={{ fontWeight: 600, opacity: 0.7 }}>(sent to driver)</span>
                  </label>
                  ...
                  <button onClick={handleReject} disabled={acting || !rejectNote.trim()}
                    ...
                    {acting ? 'Processing...' : '❌ Reject & Notify Driver'}
                  </button>
                </div>
              </>
            )}
            ...
            {tab === 'ACTIVE' && (
              <button onClick={() => handleSuspend(selected)} disabled={acting}
                style={{ width: '100%', padding: '13px', background: '#ef4444', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: 900, fontSize: '14px', cursor: 'pointer' }}>
                🚫 Suspend Driver
              </button>
            )}
            ...
            {tab === 'SUSPENDED' && (
              <button onClick={() => handleReactivate(selected)} disabled={acting}
                style={{ width: '100%', padding: '13px', background: '#22c55e', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: 900, fontSize: '14px', cursor: 'pointer' }}>
                ✅ Reactivate Driver
              </button>
            )}
          </div>
