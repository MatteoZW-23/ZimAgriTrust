...
export default function LineChart({
 ...
 return (
 <div style={{ position: "relative", width: "100%", height, minHeight: "240px" }}><svg
 viewBox="0 0 600 300"
 ...
 return (
 <g key={i}><line x1={padding.left} x2={padding.left + graphWidth} y1={y} y2={y} stroke="var(--border)" strokeDasharray="4 4" /><text x={padding.left - 10} y={y} textAnchor="end" alignmentBaseline="middle" fontSize="11" fill="var(--muted)">{formatValue(val)}
 </text></g>);
 })}
...
{hoveredData && (
 ...
 ><div style={{ opacity: 0.7 }}>{String(hoveredData.data[xKey])}</div><div style={{ fontWeight: 'bold', color: 'var(--success)' }}>${hoveredData.data[yKey].toLocaleString()}</div></div>)}
 </div>);
}
