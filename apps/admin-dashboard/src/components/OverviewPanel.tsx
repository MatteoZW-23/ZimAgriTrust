...
export function OverviewPanel({ overview, profile, onSync, onViewChange, token, activities }) {
 ...
 return (
 <div className="main-content-v4">{/* ROLE HEADER */}
 <header style={{ display: 'flex', alignItems: 'center', gap: '20px', marginBottom: '32px', padding: '28px 32px', background: 'var(--v4-surface)', borderRadius: '24px', border: '1.5px solid var(--v4-border)' }}><div style={{ width: '56px', height: '56px', borderRadius: '16px', background: meta.color, color: '#fff', display: 'grid', placeItems: 'center', fontSize: '22px', flexShrink: 0 }}><i className={`fas ${meta.icon}`}></i></div><div style={{ flex: 1 }}><h1 style={{ margin: 0, fontSize: '22px', fontWeight: 900 }}>{meta.title}</h1><p style={{ margin: '4px 0 0', fontSize: '13px', color: 'var(--v4-text-dim)', fontWeight: 600 }}>{meta.sub}</p></div><div style={{ display: 'flex', gap: '8px' }}><button className="q-btn ghost small" onClick={onSync} style={{ border: '1.5px solid var(--v4-border)' }}><i className="fas fa-rotate"></i> Sync
 </button></div></header>
{/* KPI STRIP */}
 <div className="v4-stats-grid">{kpis.map((kpi, i) => (
 <div key={i} className="v4-kpi-card hover-lift"><div className="kpi-icon" style={{ color: kpi.color }}><i className={`fas ${kpi.icon}`}></i></div><div className="kpi-data"><label>{kpi.label}</label><strong>{kpi.value}</strong></div></div>))}
 </div>
{/* QUICK ACTIONS */}
 <div style={{ marginBottom: '32px' }}><h3 style={{ fontSize: '13px', fontWeight: 900, color: 'var(--v4-text-dim)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '16px' }}>Quick Actions</h3><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px' }}>{actions.map((action, i) => (
 <button key={i} onClick={() => onViewChange(action.view)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '16px 20px', background: 'var(--v4-surface)', border: '1.5px solid var(--v4-border)', borderRadius: '16px', cursor: 'pointer', transition: 'all 0.2s' }}><div style={{ width: '36px', height: '36px', borderRadius: '10px', background: `${action.color}15`, color: action.color, display: 'grid', placeItems: 'center', fontSize: '14px' }}><i className={`fas ${action.icon}`}></i></div><span style={{ fontSize: '12px', fontWeight: 800, color: 'var(--v4-text-main)' }}>{action.label}</span></button>))}
 </div></div>
{/* MARKET PULSE + REGIONAL DATA GRID */}
 <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>{/* Price Trend Chart — visible to all except SUPPORT_ADMIN */}
 {effectiveRole !== 'SUPPORT_ADMIN' && (
 <div className="v4-glass-card-premium" style={{ padding: '24px' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}><div><h4 style={{ margin: 0, fontSize: '15px', fontWeight: 950 }}>Price Parity Index</h4><p style={{ margin: 0, fontSize: '11px', color: 'var(--v4-text-dim)', fontWeight: 700 }}>30-day staple commodity trend</p></div><div style={{ fontSize: '16px', fontWeight: 1000, color: ppiChange !== null ? (Number(ppiChange) >= 0 ? '#20963D' : '#ef4444') : '#94a3b8' }}>{ppiChange !== null ? `${Number(ppiChange) >= 0 ? '+' : ''}${ppiChange}%` : '—'}
 </div></div><LineChart data={trendData} xKey="label" yKey="value" color="#20963D" height={180} /></div>)}
 ...
 <div className="v4-glass-card-premium" style={{ padding: '24px' }}><h4 style={{ margin: '0 0 20px', fontSize: '15px', fontWeight: 950 }}>Regional Activity</h4><div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>{regionalData.length > 0 ? regionalData.slice(0, 5).map((r, i) => {
 ...
 return (
 <div key={r.province || i}><div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', fontWeight: 700, marginBottom: '6px' }}><span>{(r.province || 'Other').toUpperCase()}</span><span style={{ color: colors[i % colors.length] }}>{r.listings || 0} listings</span></div><div style={{ height: '4px', background: 'var(--v4-border)', borderRadius: '10px', overflow: 'hidden' }}><div style={{ width: `${pct}%`, height: '100%', background: colors[i % colors.length], transition: 'width 0.6s ease' }}></div></div></div>);
 }) : (
 ...
 <div className="v4-glass-card-premium" style={{ padding: '24px' }}><h4 style={{ margin: '0 0 20px', fontSize: '15px', fontWeight: 950 }}>Dispute Overview</h4><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}><div style={{ padding: '16px', background: 'rgba(239,68,68,0.06)', borderRadius: '14px', border: '1px solid rgba(239,68,68,0.15)' }}><div style={{ fontSize: '10px', color: '#94a3b8', fontWeight: 800, marginBottom: '4px' }}>OPEN</div><div style={{ fontSize: '22px', fontWeight: 950, color: '#ef4444' }}>{overview?.stats?.open_disputes ?? 0}</div></div><div style={{ padding: '16px', background: 'rgba(16,185,129,0.06)', borderRadius: '14px', border: '1px solid rgba(16,185,129,0.15)' }}><div style={{ fontSize: '10px', color: '#94a3b8', fontWeight: 800, marginBottom: '4px' }}>RESOLVED</div><div style={{ fontSize: '22px', fontWeight: 950, color: '#10b981' }}>{overview?.stats?.resolved_disputes ?? 0}</div></div></div><button onClick={() => onViewChange('disputes')} style={{ width: '100%', marginTop: '16px', padding: '12px', borderRadius: '12px', background: 'rgba(239,68,68,0.08)', color: '#ef4444', border: 'none', fontWeight: 800, fontSize: '12px', cursor: 'pointer' }}>View Dispute Queue
 </button></div>)}

 {/* System Status — visible to SYSTEM_ADMIN */}
 {effectiveRole === 'SYSTEM_ADMIN' && (
 <div className="v4-glass-card-premium" style={{ padding: '24px' }}><h4 style={{ margin: '0 0 20px', fontSize: '15px', fontWeight: 950 }}>Service Health</h4><div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>{[
 ...
 <div key={svc.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', background: 'var(--v4-bg)', borderRadius: '12px', border: '1px solid var(--v4-border)' }}><span style={{ fontSize: '12px', fontWeight: 700 }}>{svc.name}</span><div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '11px', fontWeight: 900, color: svc.color }}><div style={{ width: '6px', height: '6px', borderRadius: '50%', background: svc.color }}></div>{svc.status}
 </div></div>))}
 </div></div>)}
 </div>
{/* RECENT ACTIVITY FEED */}
 <div style={{ marginTop: '32px' }}><h3 style={{ fontSize: '13px', fontWeight: 900, color: 'var(--v4-text-dim)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '16px' }}>Recent Platform Activity</h3><div className="v4-glass-card-premium" style={{ padding: '20px' }}>{mergedActivities.length > 0 ? mergedActivities.slice(0, 6).map((act, i) => (
 <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '12px 0', borderBottom: i < 5 ? '1px solid var(--v4-border)' : 'none' }}><div style={{ fontSize: '8px', fontWeight: 950, padding: '4px 10px', borderRadius: '100px', background: act.actType === 'OFFER' ? 'rgba(59,130,246,0.1)' : act.actType === 'DEAL' ? 'rgba(16,185,129,0.1)' : 'rgba(148,163,184,0.1)', color: act.actType === 'OFFER' ? '#3b82f6' : act.actType === 'DEAL' ? '#20963D' : '#64748b', minWidth: '60px', textAlign: 'center' }}>{act.actType}</div><div style={{ flex: 1 }}><p style={{ margin: 0, fontSize: '13px', fontWeight: 700, color: 'var(--v4-text-main)' }}>{act.commodity || act.title || 'Trade Activity'}</p><span style={{ fontSize: '11px', color: 'var(--v4-text-dim)' }}>{act.quantity || act.amount || '--'} {act.unit || 'units'}</span></div><span style={{ fontSize: '10px', fontWeight: 800, color: 'var(--v4-text-dim)' }}>{act.time}</span></div>)) : (
 <div style={{ padding: '40px 0', textAlign: 'center' }}><i className="fas fa-inbox" style={{ fontSize: '32px', color: 'var(--v4-border)', marginBottom: '12px', display: 'block' }}></i><p style={{ color: 'var(--v4-text-dim)', fontWeight: 700, fontSize: '13px' }}>No recent activity.</p></div>)}
 </div></div></div>);
}
