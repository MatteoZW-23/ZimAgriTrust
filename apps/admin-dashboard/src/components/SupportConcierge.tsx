...
export default function SupportConcierge({ profile, onViewChange }) {
 ...
 return (
 <><div className={`concierge-bubble ${isOpen ? 'active' : ''}`} onClick={() => !isOpen && setIsOpen(true)}><div className="bubble-aura"></div><i className={`fas ${isOpen ? 'fa-times' : 'fa-headset'}`}></i></div>
<div className={`concierge-window ${isOpen ? 'open' : ''}`}><div className="concierge-header"><div className="c-avatar"><i className="fas fa-user-tie"></i><div className="online-status"></div></div><div className="c-meta"><strong>ZimAgritrust Concierge</strong><span>Platform Support Agent</span></div><button className="c-close" onClick={() => setIsOpen(false)}><i className="fas fa-chevron-down"></i></button></div>
<div className="concierge-body" ref={scrollRef}>{messages.map(m => (
 <div key={m.id} className={`msg-row ${m.sender}`}><div className="msg-bubble">{m.text}
 <span className="msg-time">{m.time}</span></div></div>))}
 {isTyping && (
 <div className="msg-row bot"><div className="msg-bubble shimmer" style={{ opacity: 0.7, padding: '14px 20px' }}><div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}><div className="type-dot"></div><div className="type-dot"></div><div className="type-dot"></div><span style={{ fontSize: '9px', fontWeight: 900, marginLeft: '8px', opacity: 0.5 }}>SYNCHRONIZING DATA...</span></div></div></div>)}
 </div>
<div className="concierge-footer"><div className="quick-actions">{quickActions.map((qa, i) => (
 <button key={i} className="qa-chip" onClick={() => {
 handleSend(qa.label);
 onViewChange(qa.view);
 }}>{qa.label}</button>))}
 </div><div className="input-area"><input 
 type="text" 
 placeholder="Type support details..." 
 value={inputValue} 
 onChange={(e) => setInputValue(e.target.value)} 
 onKeyPress={(e) => e.key === 'Enter' && handleSend()}
 /><button className="send-btn" onClick={() => handleSend()}><i className="fas fa-paper-plane"></i></button></div></div></div>
<style>{`
 .concierge-bubble {
 position: fixed;
 bottom: 30px;
