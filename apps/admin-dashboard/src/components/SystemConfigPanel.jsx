...
function StatusBadge({ status }) {
  ...
  return (
    <span style={{
      ...
      <i className={`fas ${t.icon}`} style={{ fontSize: 8 }} />
      {t.label}
    </span>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 15, fontWeight: 900, color: 'var(--v4-text-main)', letterSpacing: '-0.01em' }}>{title}</div>
          {subtitle && <div style={{ fontSize: 12, color: 'var(--v4-text-dim)', marginTop: 2, fontWeight: 500 }}>{subtitle}</div>}
        </div>
      </div>
...
function FieldRow({ label, hint, children }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 24, padding: '14px 0', borderBottom: '1px solid var(--v4-border)' }}>
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--v4-text-main)' }}>{label}</div>
        {hint && <div style={{ fontSize: 11, color: 'var(--v4-text-dim)', marginTop: 2 }}>{hint}</div>}
      </div>
  ...
  return (
    <div style={{
      ...
      <i className={`fas ${type === 'success' ? 'fa-check-circle' : type === 'error' ? 'fa-times-circle' : 'fa-info-circle'}`}
         style={{ color: colors[type], fontSize: 18 }} />
      <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--v4-text-main)', flex: 1 }}>{message}</span>
      <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--v4-text-dim)', fontSize: 16 }}>×</button>
    </div>
  );
}
...
export default function SystemConfigPanel({ token }) {
  ...
  if (loading) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 400, gap: 16 }}>
      <div style={{ width: 40, height: 40, border: '3px solid var(--v4-border)', borderTopColor: 'var(--v4-accent)', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
      <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--v4-text-dim)' }}>Synchronising platform parameters…</span>
    </div>
  );
    ...
    <div className="main-content-v4 animate-fade-in">
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <span style={{ fontSize: 10, fontWeight: 900, letterSpacing: '0.15em', color: 'rgba(255,255,255,0.45)', textTransform: 'uppercase' }}>
            Institutional Governance
          </span>
          <h1 style={{ margin: 0, fontSize: 30, fontWeight: 950, color: '#fff', letterSpacing: '-0.03em', lineHeight: 1.1 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20, alignItems: 'flex-end' }}>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 9, fontWeight: 900, letterSpacing: '0.12em', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', marginBottom: 6 }}>System Status</div>
            <StatusBadge status={isLocked ? 'LOCKED' : 'OPERATIONAL'} />
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 9, fontWeight: 900, letterSpacing: '0.12em', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', marginBottom: 6 }}>Environment</div>
            <span style={{ fontSize: 14, fontWeight: 900, color: '#fff' }}>Production Cluster</span>
          </div>
        </div>
      </div>
      ...
      {isLocked && (
        <div style={{
          ...
          <div style={{ width: 36, height: 36, borderRadius: 10, background: '#ef4444', display: 'grid', placeItems: 'center', color: '#fff', fontSize: 16, flexShrink: 0 }}>
            <i className="fas fa-lock" />
          </div>
          <div>
            <div style={{ fontSize: 14, fontWeight: 900, color: '#991b1b' }}>Emergency Lockdown Active</div>
            <div style={{ fontSize: 12, color: '#b91c1c', marginTop: 2 }}>Platform is in read-only mode. All write operations are suspended.</div>
          </div>
          <ActionButton label="Release Lockdown" icon="fa-unlock" variant="danger" onClick={handleToggleLockdown} disabled={saving === 'lockdown'} />
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 28, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <SectionCard icon="fa-coins" iconColor="#f59e0b" title="Economic Architecture" subtitle="Fees, escrow release, and currency settlement rules." accent="#f59e0b">
            <FieldRow label="Global Marketplace Fee" hint="Applied to all completed transactions">
              <NumberInput
                value={getConfig('marketplace_fee', '1.0')} step={0.1} suffix="%"
                onCommit={v => handleUpdate('marketplace_fee', v)}
              />
            </FieldRow>
            <FieldRow label="Minimum Escrow Fee" hint="Floor fee regardless of transaction size">
              <NumberInput
                value={getConfig('min_escrow_fee', '0.5')} step={0.1} prefix="$"
                onCommit={v => handleUpdate('min_escrow_fee', v)}
              />
            </FieldRow>
            <FieldRow label="Default Escrow Hold Duration" hint="Days before automatic release">
              <NumberInput
          ...
          {/* Intelligence & Trust */}
          <SectionCard icon="fa-brain" iconColor="#8b5cf6" title="Intelligence & Trust" subtitle="AI verification parameters and risk scoring thresholds." accent="#8b5cf6">
            <Toggle
          ...
          {/* Regional Nodes */}
          <SectionCard icon="fa-map-marked-alt" iconColor="#0ea5e9" title="Regional Node Configuration" subtitle="Province-level marketplace and logistics settings." accent="#0ea5e9">
            <Toggle
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {/* Security Controls */}
          <SectionCard icon="fa-shield-alt" iconColor="#ef4444" title="Security Controls" subtitle="Platform protection and governance actions.">
          ...
          {/* WhatsApp Bridge */}
          <SectionCard icon="fa-comment-dots" iconColor="#10b981" title="WhatsApp Bridge" subtitle="Farmer messaging and AI crop scan channel.">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--v4-text-dim)' }}>Connection</span>
                <StatusBadge status={waStatus.status} />
              </div>
              {waStatus.phone_number && (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--v4-text-dim)' }}>Number</span>
                  <span style={{ fontSize: 12, fontWeight: 900, color: 'var(--v4-text-main)', fontFamily: 'monospace' }}>{waStatus.phone_number}</span>
                </div>
              )}
              {waStatus.reason && (
                <div style={{ fontSize: 11, color: 'var(--v4-text-dim)', background: 'var(--v4-bg)', padding: '8px 12px', borderRadius: 8, lineHeight: 1.5 }}>
                  {waStatus.reason}
                </div>
              )}
              <a
          ...
          {/* Audit Log */}
          <SectionCard icon="fa-history" iconColor="#64748b" title="Audit Log" subtitle="Recent configuration changes.">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              {configs.filter(c => c.updated_at).slice(0, 5).map((c, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: i < 4 ? '1px solid var(--v4-border)' : 'none' }}>
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--v4-text-main)' }}>{c.key.replace(/_/g, ' ')}</div>
                    <div style={{ fontSize: 10, color: 'var(--v4-text-dim)', marginTop: 2 }}>{c.updated_at ? new Date(c.updated_at).toLocaleDateString() : '—'}</div>
                  </div>
                  <span style={{ fontSize: 11, fontWeight: 900, color: 'var(--v4-text-dim)', fontFamily: 'monospace', background: 'var(--v4-bg)', padding: '2px 8px', borderRadius: 6 }}>
                    {String(c.value).slice(0, 12)}
                  </span>
                </div>
              ))}
