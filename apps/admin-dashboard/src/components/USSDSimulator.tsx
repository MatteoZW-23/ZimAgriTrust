...
export default function USSDSimulator({ profile }) {
 ...
 return (
 <div className="ussd-master-container animate-fade"><div className="ussd-header-spec"><h2>USSD Bridge Diagnostics</h2><p>Real-time emulation and SS7 network payload tracing for mobile-first agricultural transactions.</p></div>
<div className="ussd-split-view"><div className="phone-bezel"><div className="phone-screen"><div className="screen-status-bar"><span className="carrier">ECONET </span><span className="time">12:45 PM</span><span className="battery">88% </span></div>
 <div className="ussd-body">{isProcessing ? (
 <div className="ussd-loading"><div className="spinner"></div><p>Connecting to ZimAgritrust Escrow...</p></div>) : (
 <><div className="ussd-title">{activeMenu.title.split('\n').map((line, i) => <div key={i}>{line}</div>)}
 </div><div className="ussd-options">{activeMenu.options.map((opt, i) => (
 <div key={i} className="ussd-option-row">{opt}</div>))}
 </div><div className="ussd-input-area"><input 
 ref={inputRef}
 type="text" 
 value={input} 
 ...
 /><div className="ussd-actions"><button onClick={() => handleInput(input)}>{isEndScreen ? 'DIAL' : 'SEND'}</button><button onClick={cancelSession}>CANCEL</button></div></div></>)}
 </div></div><div className="phone-buttons"><div className="home-btn"></div></div></div>
<div className="ussd-tracer"><div className="tracer-header"><span style={{ fontWeight: 850, fontSize: '11px', letterSpacing: '0.05em' }}><i className="fas fa-network-wired"></i> LIVE SS7 PAYLOAD INTERCEPT</span><div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}><span style={{ fontSize: '10px', color: '#64748b' }}>PORT: 8080</span><div className="pulse-dot active" style={{ width: '6px', height: '6px' }}></div></div></div><div className="tracer-body">{history.length === 0 ? (
 <div className="tracer-empty">Awaiting USSD dialectic connection bridge... (*123#)</div>) : (
 history.filter(h => h.type === 'tx').map((h, i) => (
 <div key={i} className="tracer-log"><div className="log-meta"><span className="log-time">{new Date().toISOString().split('T')[1].slice(0,-1)}</span><span className="log-type">[POST] /v1/ussd/session</span></div><pre className="log-json">
{JSON.stringify({
