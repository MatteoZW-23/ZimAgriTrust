...
export default function MarketIntelligence({ token }) {
  ...
  if (entries.length === 0) {
    return (
      <div className="v4-glass-card-premium animate-rise" style={{ marginTop: '24px', padding: '48px', textAlign: 'center', opacity: 0.6 }}>
        <i className="fas fa-chart-line" style={{ fontSize: '40px', marginBottom: '16px', color: 'var(--v4-text-dim)' }}></i>
        <p style={{ fontWeight: 800 }}>No market price data available yet. Prices will appear once listings and transactions are recorded.</p>
      </div>
    );
  }
      ...
      <div className="v4-card-header" style={{ marginBottom: '32px' }}>
        <div>
          <h3 style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <i className="fas fa-chart-line" style={{ color: 'var(--v4-primary)' }}></i> Market Price Advisor
          </h3>
          <p style={{ fontSize: '13px', color: 'var(--v4-text-dim)', marginTop: '4px', fontWeight: 600 }}>Current commodity prices from platform listings and transactions.</p>
        </div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '24px' }}>
        {entries.map(([crop, data]) => {
            ...
            <div key={crop} className="v4-market-node" style={{ background: 'var(--v4-bg)', padding: '24px', borderRadius: '24px', border: '1.5px solid var(--v4-border)', transition: '0.3s cubic-bezier(0.4, 0, 0.2, 1)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: '24px', fontWeight: 1000, color: 'var(--v4-primary)', letterSpacing: '-0.02em' }}>${currentPrice}/{unit}</div>
                  <div style={{ fontSize: '10px', color: 'var(--v4-text-dim)', fontWeight: 800 }}>Current avg. price</div>
                </div>
              </div>
              ...
              <div style={{ height: '150px', width: '100%', marginBottom: '20px', background: 'rgba(255,255,255,0.02)', borderRadius: '16px', overflow: 'hidden' }}>
                {hasTrends ? (
                  ...
                  <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0.4, fontSize: '12px', fontWeight: 700 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '16px', paddingTop: '16px', borderTop: '1px dotted var(--v4-border)' }}>
                <div style={{ fontSize: '9px', fontWeight: 900, color: 'var(--v4-text-dim)', textTransform: 'uppercase' }}>
                  Source: {data?.origin || 'Platform listings'}
                </div>
                <div style={{ fontSize: '9px', fontWeight: 900, color: 'var(--v4-text-dim)', fontFamily: 'monospace', opacity: 0.5 }}>
                  {hasTrends ? `${cropTrends.length} data points` : 'Live price'}
                </div>
              </div>
            </div>
          );
