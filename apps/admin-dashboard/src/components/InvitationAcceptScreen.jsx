...
export default function InvitationAcceptScreen({ onComplete }) {
    ...
    return (
      <div style={containerStyle}>
        <div style={cardStyle}>
          ...
          <p>Welcome to ZimAgriTrust{acceptedRole ? ` as ${acceptedRole}` : ""}.</p>
          <button onClick={onComplete} style={btnPrimary}>Go to Login</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={containerStyle}>
      <div style={cardStyle}>
        <h2 style={{ marginTop: 0 }}>👑 Activate Your Admin Account</h2>
        <p style={{ color: "#666" }}>
          Set a strong password and verify your details to complete onboarding.
        </p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Field label="Phone Number" value={form.phone_number} onChange={(v) => update("phone_number", v)} placeholder="+263..." required />
          <Field
