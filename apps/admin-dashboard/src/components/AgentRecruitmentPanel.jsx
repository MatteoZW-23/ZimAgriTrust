...
export default function AgentRecruitmentPanel({ token }) {
    ...
    return (
        <div className="v4-recruitment-pipeline animate-fade-in">
            <header className="recruitment-hero-v4">
                ...
                <div className="hero-stats">
                    ...
                    <div className="stat-pill">
                        <label>READY FOR CERT</label>
                        <strong>{applications.filter(a => a.status === 'READY_FOR_CERTIFICATION').length}</strong>
                    </div>
                </div>
            </header>
            ...
            <div className="v4-pipeline-grid">
                {applications.map(app => (
                    <div key={app.id} className={`v4-applicant-card ${app.status === 'CERTIFIED' ? 'archived' : ''}`}>
                        <div className="card-top">
                            <div className="candidate-info">
                                <div className="c-av" style={{ background: 'var(--v4-primary-gradient)' }}>{app.full_name?.charAt(0)}</div>
                                <div className="c-meta">
                                    <strong>{app.full_name}</strong>
                                    <span>{app.phone_number} // {app.province}</span>
                                </div>
                            </div>
                            <div className={`status-tag ${app.status?.toLowerCase()}`}>{app.status?.replace(/_/g, ' ')}</div>
                        </div>
                        ...
                        <div className="pipeline-stepper" style={{ gap: '4px' }}>
                            {[
                                ...
                                return (
                                    <div key={step.id} className={`step-node ${isDone ? 'done' : ''} ${isActive ? 'active' : ''}`} style={{ width: '40px' }}>
                                        <div className="node-icon" style={{ width: '24px', height: '24px', fontSize: '10px' }}><i className={`fas ${step.icon}`}></i></div>
                                        <label style={{ fontSize: '7px' }}>{step.label}</label>
                                    </div>
                                );
                            })}
                        </div>
                        ...
                        <div className="progress-details">
                            <div className="p-item">
                                ...
                                <div className="p-bar-v4">
                                    <div className="p-fill" style={{ width: `${Math.min(100, (app.training_modules_completed?.filter(m => m.startsWith('M')).length || 0) * 10)}%` }}></div>
                                </div>
                                <span className="p-count">{Math.min(10, (app.training_modules_completed?.filter(m => m.startsWith('M')).length || 0))}/10 MODULES</span>
                            </div>
                        </div>

                        <div className="card-actions">
                            {app.status === 'APPLIED' && (
                                <button className="v4-action-btn primary" onClick={() => handleAction(app.id, 'DOCS')}>
                                    <i className="fas fa-file-signature"></i> VERIFY DOCS
                                </button>
                            )}
                            {app.status === 'DOCS_VERIFIED' && (
                                <div className="p-item" style={{ fontSize: '11px', color: 'var(--v4-text-dim)', fontStyle: 'italic' }}>
                                    Awaiting curriculum sync...
                                </div>
                            )}
                            {app.status === 'DOCS_VERIFIED' && app.training_modules_completed?.length >= 5 && (
                                <button className="v4-action-btn primary" onClick={() => handleAction(app.id, 'GEAR')}>
                                    <i className="fas fa-microchip"></i> ISSUE EQUIPMENT
                                </button>
                            )}
                            {app.status === 'EXAM_PASSED' && (
                                <button className="v4-action-btn primary" onClick={() => handleAction(app.id, 'PRACTICAL')}>
                                    <i className="fas fa-vial"></i> LOG PRACTICAL
                                </button>
                            )}
                            {app.status === 'READY_FOR_SHADOWING' && (
                                <button className="v4-action-btn warning" onClick={() => handleAction(app.id, 'SHADOW')}>
                                    <i className="fas fa-user-ninja"></i> LOG SHADOWING
                                </button>
                            )}
                            {app.status === 'SUPERVISED_INDEPENDENT' && (
                                <button className="v4-action-btn success" onClick={() => handleAction(app.id, 'CERTIFY')}>
                                    <i className="fas fa-check-double"></i> FINAL SIGN-OFF
                                </button>
                            )}
                            {app.status === 'READY_FOR_CERTIFICATION' && (
                                <button className="v4-action-btn success" onClick={() => handleAction(app.id, 'CERTIFY')}>
                                    <i className="fas fa-certificate"></i> CERTIFY AGENT
                                </button>
                            )}
                            {app.status === 'CERTIFIED' && (
                                <div className="certified-badge">
                                    <i className="fas fa-check-double"></i> AGENT ACTIVATED
                                </div>
                            )}
                        </div>
                    </div>
