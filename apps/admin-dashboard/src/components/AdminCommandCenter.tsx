...
Type any command and press Enter to execute.
 ...
 const { health, whatsapp, models } = systemStatus;
...
Last Updated: ${new Date(systemStatus.timestamp).toLocaleString()}
 ...
 const rows = users.slice(0, 20).map(u => String(u.id).padEnd(10) +
 ...
 const rows = txns.slice(0, 20).map(t => String(t.id || t.order_id).slice(0, 14).padEnd(15) +
 ...
 const rows = logs.slice(0, 15).map(log => `[${new Date(log.timestamp).toLocaleString()}] ${log.level?.toUpperCase()} - ${log.message || log.details || 'N/A'}`
 );
 ...
 const rows = diag.checks.map(check => check.name.padEnd(30) +
...
Timestamp: ${new Date(stats.timestamp).toLocaleString()}
 ...
 return (
 <div className="command-center-shell"><div className="cc-header"><div className="cc-title"><i className="fas fa-terminal"></i><h2>Admin Command Center</h2><span className="cc-badge">PRIVILEGED ACCESS</span></div><div className="cc-tabs"><button 
...
{activeTab === 'terminal' && (
 <div className="cc-terminal-view"><div className="cc-quick-actions">{quickActions.map((action, idx) => (
 <button
 key={idx}
 ...
 ><i className={`fas ${action.icon}`} style={{ color: action.color }}></i><span>{action.label}</span></button>))}
 </div>
<div className="cc-terminal" ref={terminalRef}><div className="cc-terminal-header"><span className="cc-terminal-title"><i className="fas fa-circle" style={{ color: '#10b981' }}></i>ZimAgritrust Command Terminal
 </span><span className="cc-terminal-info">{new Date().toLocaleString()} | Session Active
 </span></div>
<div className="cc-terminal-content">{commandHistory.length === 0 && (
 <div className="cc-welcome"><pre>{`
╔════════════════════════════════════════════════════════════╗
...
╚════════════════════════════════════════════════════════════╝
 ...
 <div key={idx} className={`cc-entry cc-entry-${entry.type}`}><div className="cc-command-line"><span className="cc-prompt">admin@zimagritrust:~$</span><span className="cc-command">{entry.command}</span><span className="cc-timestamp">{new Date(entry.timestamp).toLocaleTimeString()}</span></div><pre className="cc-output">{entry.output}</pre></div>))}

 {loading && (
 <div className="cc-loading"><i className="fas fa-spinner fa-spin"></i> Processing...
 </div>)}
 </div>
<div className="cc-input-line"><span className="cc-prompt">admin@zimagritrust:~$</span><input
 type="text"
 className="cc-input"
 ...
 {activeTab === 'system' && (
 <div className="cc-system-view"><div className="cc-system-grid"><div className="cc-system-card"><div className="cc-card-header"><i className="fas fa-server"></i><h3>Platform Metrics</h3></div><div className="cc-card-body">{systemStatus ? (
 <><div className="cc-metric"><span>Total Users</span><strong>{systemStatus.health?.metrics?.users?.total || 0}</strong></div><div className="cc-metric"><span>Active Listings</span><strong>{systemStatus.health?.metrics?.listings?.active || 0}</strong></div><div className="cc-metric"><span>Pending Orders</span><strong>{systemStatus.health?.metrics?.orders?.pending || 0}</strong></div><div className="cc-metric"><span>Escrow Value</span><strong>${systemStatus.health?.metrics?.financial?.escrow_value?.toFixed(2) || '0.00'}</strong></div></>) : (
 <div className="cc-loading">Loading...</div>)}
 </div></div>
<div className="cc-system-card"><div className="cc-card-header"><i className="fas fa-network-wired"></i><h3>Services Status</h3></div><div className="cc-card-body">{systemStatus ? (
 <><div className="cc-service"><span>Database</span><span className={`cc-status cc-status-${systemStatus.health?.database?.status === 'healthy' ? 'online' : 'offline'}`}>{systemStatus.health?.database?.status || 'unknown'}
 </span></div><div className="cc-service"><span>WhatsApp Service</span><span className={`cc-status cc-status-${systemStatus.whatsapp?.status === 'connected' ? 'online' : 'offline'}`}>{systemStatus.whatsapp?.status || 'unknown'}
 </span></div><div className="cc-service"><span>AI Models</span><span className="cc-status cc-status-online">{systemStatus.models?.length || 0} loaded
 </span></div><div className="cc-service"><span>Agent Network</span><span className="cc-status cc-status-online">{systemStatus.health?.metrics?.agents?.active || 0} active
 </span></div></>) : (
 <div className="cc-loading">Loading...</div>)}
 </div></div>
<div className="cc-system-card cc-card-wide"><div className="cc-card-header"><i className="fas fa-chart-line"></i><h3>Real-Time Monitoring</h3></div><div className="cc-card-body"><div className="cc-monitor-grid"><div className="cc-monitor-item"><i className="fas fa-users" style={{ color: '#3b82f6' }}></i><div><span>Active Users</span><strong>{systemStatus?.health?.metrics?.users?.active || 0}</strong></div></div><div className="cc-monitor-item"><i className="fas fa-handshake" style={{ color: '#10b981' }}></i><div><span>Active Agents</span><strong>{systemStatus?.health?.metrics?.agents?.active || 0}</strong></div></div><div className="cc-monitor-item"><i className="fas fa-box" style={{ color: '#f59e0b' }}></i><div><span>Active Listings</span><strong>{systemStatus?.health?.metrics?.listings?.active || 0}</strong></div></div><div className="cc-monitor-item"><i className="fas fa-wallet" style={{ color: '#8b5cf6' }}></i><div><span>Escrow Holdings</span><strong>${systemStatus?.health?.metrics?.financial?.escrow_value?.toFixed(2) || '0.00'}</strong></div></div></div></div></div></div>
<button className="cc-refresh-btn" onClick={loadSystemStatus}><i className="fas fa-sync"></i> Refresh Status
 </button></div>)}

 {activeTab === 'operations' && (
 <div className="cc-operations-view"><div className="cc-ops-grid"><div className="cc-ops-card"><div className="cc-ops-icon" style={{ background: '#3b82f6' }}><i className="fas fa-sync"></i></div><h3>Platform Sync</h3><p>Synchronize all platform data and refresh caches</p><button 
 className="cc-ops-btn"
 onClick={() => executeCommand('sync')}
 disabled={loading}
 >Execute Sync
 </button></div>
<div className="cc-ops-card"><div className="cc-ops-icon" style={{ background: '#10b981' }}><i className="fas fa-calculator"></i></div><h3>Reconcile Platform</h3><p>Reconcile financial records and escrow balances</p><button 
 className="cc-ops-btn"
 onClick={() => executeCommand('reconcile')}
 disabled={loading}
 >Run Reconciliation
 </button></div>
<div className="cc-ops-card"><div className="cc-ops-icon" style={{ background: '#8b5cf6' }}><i className="fas fa-star"></i></div><h3>Recompute Trust</h3><p>Recalculate trust scores for all users</p><button 
 className="cc-ops-btn"
 onClick={() => executeCommand('recompute-trust')}
 disabled={loading}
 >Recompute Scores
 </button></div>
<div className="cc-ops-card"><div className="cc-ops-icon" style={{ background: '#06b6d4' }}><i className="fas fa-broom"></i></div><h3>Database Cleanup</h3><p>Clean up old data and optimize storage</p><button 
 className="cc-ops-btn"
 onClick={() => executeCommand('maintenance cleanup')}
 disabled={loading}
 >Run Cleanup
 </button></div>
<div className="cc-ops-card"><div className="cc-ops-icon" style={{ background: '#f59e0b' }}><i className="fas fa-gauge-high"></i></div><h3>Optimize Database</h3><p>Optimize database queries and indexes</p><button 
 className="cc-ops-btn"
 onClick={() => executeCommand('maintenance optimize')}
 disabled={loading}
 >Optimize Now
 </button></div>
<div className="cc-ops-card"><div className="cc-ops-icon" style={{ background: '#14b8a6' }}><i className="fas fa-stethoscope"></i></div><h3>System Diagnostics</h3><p>Run comprehensive system health checks</p><button 
 className="cc-ops-btn"
 onClick={() => executeCommand('diagnostics')}
 disabled={loading}
 >Run Diagnostics
 </button></div>
<div className="cc-ops-card cc-ops-danger"><div className="cc-ops-icon" style={{ background: '#ef4444' }}><i className="fas fa-lock"></i></div><h3>Emergency Lockdown</h3><p>Enable platform-wide lockdown mode</p><button 
 className="cc-ops-btn cc-ops-btn-danger"
 onClick={() => {
 if (window.confirm(' Enable emergency lockdown? This will restrict all platform operations.')) {
