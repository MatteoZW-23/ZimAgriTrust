...
function HealthCheckPanel() {
 ...
 React.useEffect(() => {
 const base = HEALTH_API.replace(/\/api\/v1\/?$/, '');
 fetch(`${base}/health/detailed`, { cache: 'no-store' })
 .then(r => r.json())
 .then(setHealth)
 .catch(() => setError(true));
 }, []);
 ...
 if (error) return (
 <div style={{ padding: '20px 24px', fontFamily: 'monospace', fontSize: '12px', color: '#ef4444' }}>Unable to reach backend health endpoint.
 </div>);
 if (!health) return (
 <div style={{ padding: '20px 24px', fontFamily: 'monospace', fontSize: '12px', color: '#64748b' }}>Checking...
 </div>);

 const checks = health.checks || {};
 const services = [
 { name: 'API Server', status: health.status || 'unknown' },
 { name: 'PostgreSQL', status: checks.postgres?.status || 'unknown', detail: checks.postgres?.latency_ms ? `${checks.postgres.latency_ms}ms` : '' },
 { name: 'Redis', status: checks.redis?.status || 'unknown', detail: checks.redis?.latency_ms ? `${checks.redis.latency_ms}ms` : '' },
 { name: 'Queues', status: checks.queues?.status || 'unknown' },
 { name: 'Payments', status: checks.payment_providers?.status || 'unknown' },
 ];

 const colorFor = (s: string) => s === 'healthy' ? '#20963D' : s === 'degraded' ? '#f59e0b' : s === 'unhealthy' ? '#ef4444' : '#64748b';
 const labelFor = (s: string) => s === 'healthy' ? 'ONLINE' : s === 'degraded' ? 'DEGRADED' : s === 'unhealthy' ? 'OFFLINE' : 'UNKNOWN';

 return (
 <><div style={{ padding: '20px 24px', fontFamily: 'monospace', fontSize: '12px' }}>{services.map(s => (
 <div key={s.name} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #1e293b' }}><span style={{ color: '#818cf8' }}>{s.name} {(s as any).detail && <span style={{ color: '#475569', marginLeft: 8 }}>({(s as any).detail})</span>}</span><span style={{ color: colorFor(s.status) }}>[{labelFor(s.status)}]</span></div>))}
 </div><div style={{ padding: '12px 24px', background: 'rgba(0,0,0,.2)', borderTop: '1.5px solid #1e293b', fontSize: '10px', color: '#444' }}>Uptime: {health.uptime_seconds ? `${Math.floor(health.uptime_seconds / 3600)}h ${Math.floor((health.uptime_seconds % 3600) / 60)}m` : '—'}
 </div></>);
}
...
function AccessDenied({ role, onLogout }) {
 ...
 }}><i className="fas fa-shield-halved" style={{ fontSize: '48px', color: '#ef4444', marginBottom: '24px' }}></i><h2 style={{ fontSize: '24px', fontWeight: 900, marginBottom: '12px' }}>Access Denied</h2><p style={{ color: '#94a3b8', marginBottom: '8px' }}>This portal is restricted to ZimAgritrust Administrators.
 </p><p style={{ color: '#64748b', fontSize: '13px', marginBottom: '32px' }}>Your current role: <strong style={{ color: '#f59e0b' }}>{role}</strong></p><p style={{ color: '#64748b', fontSize: '13px', marginBottom: '32px' }}>If you are a <strong>Farmer or Buyer</strong>, please visit{' '}
 <a href={import.meta.env.VITE_APP_URL || 'http://localhost:3003'} style={{ color: '#3b82f6' }}>the user portal
 </a>.
 {' '}If you are an <strong>Agent</strong>, visit{' '}
 <a href={import.meta.env.VITE_AGENT_URL || 'http://localhost:3001'} style={{ color: '#10b981' }}>the agent portal
 </a>.
 </p><button
 onClick={onLogout}
 style={{
 ...
 ><i className="fas fa-power-off" style={{ marginRight: '8px' }}></i>Log Out
 </button></div>);
}
...
function App() {
 ...
 const { token, profile, hasAcceptedTerms, setHasAcceptedTerms, login, logout, storeSAToken } = React.useContext(AuthContext);
 ...
 if (!token) {
 return (
 <div style={{ position: 'relative' }}>{portalMode === 'hq' ? (
 ...
 return (
 <main className={`shell-v4 theme-admin ${isSidebarCollapsed ? 'sidebar-hidden' : ''}`}>{/* Mobile overlay */}
 <div
 className={`v4-sidebar-overlay ${!isSidebarCollapsed ? 'active' : ''}`}
 onClick={() => setIsSidebarCollapsed(true)}
 />
{/* ── Sidebar ── */}
 <aside className={`sidebar-v4 ${isSidebarCollapsed ? 'collapsed' : 'mobile-open'}`}><div className="sidebar-logo-v4"><div className="logo-icon-v4"><i className="fas fa-shield-halved"></i></div>{!isSidebarCollapsed && (
 <div className="logo-text-v4"><strong>ZimAgritrust</strong><span className="platform-tag">ADMIN</span></div>)}
 <button className="v4-sidebar-toggle" onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)}><i className={`fas ${isSidebarCollapsed ? 'fa-indent' : 'fa-outdent'}`}></i></button></div>
<nav className="v4-navigation-stack">{navGroups.map((item, i) => {
 if (item.type === 'item') {
 return (
 <div
 key={item.view}
 ...
 ><i className={`fas ${item.icon}`}></i>{!isSidebarCollapsed && <span>{item.label}</span>}
 </div>);
 }
 ...
 return (
 <div key={item.key} className="nav-group-v4"><div
 className={`nav-group-header ${hasActiveChild ? 'has-active' : ''}`}
 onClick={() => isSidebarCollapsed ? null : toggleGroup(item.key)}
 title={isSidebarCollapsed ? item.label : ''}
 ><i className={`fas ${item.icon}`}></i>{!isSidebarCollapsed && (
 <><span className="group-header-label">{item.label}</span><i className={`fas fa-chevron-down nav-chevron ${isOpen ? 'open' : ''}`}></i></>)}
 </div>{!isSidebarCollapsed && isOpen && (
 <div className="nav-sub-items">{item.children.map(child => (
 <div
 key={child.view}
 className={`nav-link-v4 nav-sub-link ${currentView === child.view ? 'active' : ''}`}
 onClick={() => setCurrentView(child.view)}
 ><i className={`fas ${child.icon}`}></i><span>{child.label}</span></div>))}
 </div>)}
 {isSidebarCollapsed && (
 <div className="nav-sub-items-collapsed">{item.children.map(child => (
 <div
 key={child.view}
 className={`nav-link-v4 nav-sub-link-icon ${currentView === child.view ? 'active' : ''}`}
 onClick={() => setCurrentView(child.view)}
 title={child.label}
 ><i className={`fas ${child.icon}`}></i></div>))}
 </div>)}
 </div>);
 })}
 </nav>
<div className="v4-sidebar-footer"><div className="v4-user-mini"><div className="u-av">{profile?.full_name?.charAt(0) || 'A'}</div>{!isSidebarCollapsed && (
 <div className="u-meta"><strong>{profile.full_name}</strong><span>{effectiveRole.replace('_', ' ')}</span></div>)}
 </div><div
 className="nav-link-v4 logout-btn"
 onClick={handleLogout}
 style={{ marginTop: '10px', background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.2)' }}
 ><i className="fas fa-power-off"></i>{!isSidebarCollapsed && <span>Log Out</span>}
 </div></div></aside>
{/* ── Main content ── */}
 <section className="main-wrapper-v4">{/* Header */}
 <header className="v4-header"><div className="h-left-group"><button className="sidebar-toggle-v4" onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)}><i className="fas fa-bars-staggered"></i></button><div className="v4-hq-branding"><strong>{profile.full_name}</strong><span>Admin Workspace</span></div></div>
<div className="v4-search-bar-container"><div className="v4-search-wrapper"><i className="fas fa-search"></i><input
 type="text"
 placeholder="Search users or listings…"
 value={searchQuery}
 onChange={(e) => setSearchQuery(e.target.value)}
 /></div></div>
<div className="h-right-group"><button className="v4-icon-btn" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} title="Toggle Theme"><i className={`fas ${theme === 'dark' ? 'fa-sun' : 'fa-moon'}`}></i></button>
{/* Language */}
 <div className="v4-toggle-group" style={{ display: 'flex', gap: '4px', background: 'var(--v4-bg)', padding: '4px', borderRadius: '12px', border: '1px solid var(--v4-border)' }}>{['EN', 'SN', 'ND'].map(lang => (
 <button
 key={lang}
...
{/* Currency */}
 <div className="v4-toggle-group" style={{ display: 'flex', gap: '4px', background: 'var(--v4-bg)', padding: '4px', borderRadius: '12px', border: '1px solid var(--v4-border)' }}>{['USD', 'ZIG'].map(cur => (
 <button
 key={cur}
...
<button className="v4-icon-btn" onClick={loadAllData} title="Refresh"><i className="fas fa-rotate"></i></button>
<div className="v4-noti-v4" onClick={() => setShowNotifications(!showNotifications)}><i className="fas fa-bell"></i>{notifications.some(n => !n.read) && <div className="n-dot"></div>}
 </div>
<div className="v4-avatar-pro">{profile.full_name?.substring(0, 2).toUpperCase() || 'AD'}</div></div>
{/* Notifications dropdown */}
 {showNotifications && (
 <div className="v4-notifications-dropdown animate-rise"><div className="nd-header"><h4>Alerts</h4><button className="mark-read-btn" onClick={() => setNotifications(p => p.map(n => ({ ...n, read: true })))}>Clear all</button></div><div className="nd-body">{notifications.length > 0 ? notifications.map(n => (
 <div key={n.id} className={`nd-item ${n.read ? 'read' : 'unread'}`}
 onClick={() => setNotifications(p => p.map(x => x.id === n.id ? { ...x, read: true } : x))}><div className={`nd-icon ${n.type}`}><i className={`fas ${n.type === 'warning' ? 'fa-triangle-exclamation' : 'fa-bell'}`}></i></div><div className="nd-content"><strong>{n.title}</strong><p>{n.desc}</p><span>{n.time}</span></div></div>)) : <div className="empty-notifs">No new alerts.</div>}
 </div><div className="nd-footer" onClick={() => { setShowNotifications(false); setCurrentView('logs'); }}>View Audit Logs
 </div></div>)}
 </header>
{/* ── Views ── */}
 <div className="main-content-v4 overflow-auto">{currentView === 'overview' && (
 <div className="reality-stack-v4 animate-fade-in"><OverviewPanel overview={overview} profile={profile} onSync={loadAllData} onViewChange={setCurrentView} token={token} activities={marketActivities} /></div>)}
 {currentView === 'users' && (
 <UserDirectoryPanel
 users={users.filter(u =>u.full_name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
 u.phone_number?.includes(searchQuery)
 )}
 profile={profile} token={token} onGovernance={handleGovernance}
 />)}
 {currentView === 'suppliers' && <SupplierManagementPanel token={token} />}
 {currentView === 'marketplace' && (
 <VerificationPanel
 listings={reviewQueue.filter(l =>l.product_type?.toLowerCase().includes(searchQuery.toLowerCase()) ||
 ...
 />)}
 {currentView === 'market-monitor' && <MarketAdvisory token={token} />}
...
{/* Loading overlay */}
 {loading && (
 <div className="v4-global-loader-overlay"><div className="v4-loader-content"><div className="v4-loader-spinner shimmer"><i className="fas fa-shield-halved"></i></div><div className="v4-loader-text"><strong>Syncing…</strong><span>Fetching latest platform data.</span></div></div></div>)}
...
{/* System info modal */}
 {showSysInfo && (
 <div className="modal-overlay" onClick={() => setShowSysInfo(false)}><div className="v4-glass-card-premium animate-fade-in"
 style={{ maxWidth: '560px', background: '#020617', color: '#fff', border: '1.5px solid #1e293b', padding: 0, overflow: 'hidden' }}
 onClick={e => e.stopPropagation()}><div style={{ background: '#000E2B', padding: '20px 24px', borderBottom: '1.5px solid #1e293b', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><h3 style={{ margin: 0, fontSize: '15px', fontWeight: 900, color: '#fff' }}><i className="fas fa-heartbeat" style={{ color: '#3b82f6', marginRight: '10px' }}></i>Platform Health
 </h3><button onClick={() => setShowSysInfo(false)} style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer' }}><i className="fas fa-times"></i></button></div><HealthCheckPanel /></div></div>)}

 {/* Status bar */}
 <footer className="v4-tech-status-bar" onClick={() => setShowSysInfo(true)} style={{ cursor: 'pointer' }}><div className="status-item"><span className="dot pulse"></span> ALL SYSTEMS OPERATIONAL</div><div className="divider-v"></div><div className="status-item">ADMIN PORTAL — SECURE SESSION</div><div className="ml-auto"></div><div className="status-item opacity-50">ZimAgritrust Admin v1.0</div></footer></main>);
}
