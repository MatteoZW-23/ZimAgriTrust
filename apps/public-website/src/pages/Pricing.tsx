      ...
      <div className="container-custom">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto mb-20">
          {plans.map((plan, i) => (
            ...
            >
              {plan.popular && (
                <div className="absolute top-0 right-8 -translate-y-1/2 bg-primary-600 text-white px-4 py-1 rounded-full text-xs font-black uppercase tracking-widest">
                  Most Popular
                </div>
              )}
              <h3 className="text-2xl font-bold text-earth-800 mb-2">{plan.name}</h3>
              <div className="flex items-baseline gap-1 mb-4">
                <span className="text-5xl font-black text-earth-900">{plan.price}</span>
                {plan.period && <span className="text-earth-500 font-bold">{plan.period}</span>}
              </div>
              <p className="text-earth-600 mb-8 font-medium">{plan.description}</p>
              
              <ul className="space-y-4 mb-8">
                {plan.features.map(feat => (
                  <li key={feat} className="flex items-start gap-3 text-earth-700 font-medium">
              ...
              <button className={`w-full py-4 rounded-2xl font-black transition-all ${plan.popular ? 'bg-primary-600 text-white hover:bg-primary-700 shadow-lg shadow-primary-200' : 'bg-earth-100 text-earth-700 hover:bg-earth-200'}`}>
                {plan.cta}
              </button>
            </motion.div>
          ))}
        ...
        <div className="bg-earth-900 rounded-[3rem] p-12 text-white relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="mt-12 pt-12 border-t border-white/10 text-center">
              <p className="text-earth-400 max-w-2xl mx-auto italic">
                * All fees are automatically deducted from the payout or escrow amount. 
