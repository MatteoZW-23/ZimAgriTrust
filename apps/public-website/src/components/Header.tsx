...
<nav className="p-4 space-y-1">{navItems.map((item) => (
 ...
 <div className="pt-4 mt-4 border-t border-earth-100"><p className="px-4 text-xs font-semibold text-earth-400 uppercase tracking-wider mb-2">More</p>{dropdownItems.map((item) => (
 <Link
 key={item.to}
 to={item.to}
 onClick={() => setMobileMenuOpen(false)}
