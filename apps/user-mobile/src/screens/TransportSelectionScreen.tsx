...
import { calculateTransportQuote, requestTransport } from "../api";
...
export default function TransportSelectionScreen({ navigation, route }) {
  ...
  const handleCalculateQuote = async () => {
    ...
    try {
      const response = await calculateTransportQuote(token, {
        distance_km: Number(orderDetails?.distance_km || listing?.distance_km || 15),
        vehicle_type: selectedVehicle,
        cargo_weight_kg: Number(listing?.quantity || orderDetails?.quantity_kg || 100),
        urgency_level: urgency,
      });
      setQuote({
        ...response,
        estimated_minutes: response.estimated_minutes || response.estimated_delivery_time,
      });
    } catch (error) {
      Alert.alert('Error', error.message || 'Failed to calculate quote. Please try again.');
    } finally {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const payload = {
        ...
        mode: selectedMode,
        pickup_address: pickupAddress || listing?.location || orderDetails?.pickup_address || 'Farm pickup point',
        delivery_address: deliveryAddress || orderDetails?.delivery_address || 'Buyer delivery point',
        vehicle_type: selectedVehicle,
        preferred_vehicle_type: selectedVehicle,
        urgency_level: urgency,
        cargo_weight_kg: listing?.quantity || 100,
      };

      await requestTransport(token, payload);
