...
export default function WithdrawScreen({ navigation, route }) {
  ...
  const handleWithdraw = async () => {
    ...
    Alert.alert(
      'Confirm Withdrawal',
      `Withdraw $${parsedAmount.toFixed(2)} via ${METHODS.find(m => m.id === method)?.label}?`,
      [
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionLabel}>
            {method === 'bank' ? 'BANK ACCOUNT NUMBER' : 'MOBILE NUMBER'}
          </Text>
          <TextInput
            style={styles.input}
            placeholder={method === 'bank' ? 'e.g. 1234567890' : 'e.g. +263771234567'}
            placeholderTextColor="#94a3b8"
        ...
        {parsedAmount > 0 && (
          <View style={styles.feeCard}>
            ...
            <View style={styles.feeRow}>
              <Text style={styles.feeLabel}>Withdrawal fee (1%)</Text>
              <Text style={styles.feeValue}>-${(parsedAmount * 0.01).toFixed(2)}</Text>
            </View>
