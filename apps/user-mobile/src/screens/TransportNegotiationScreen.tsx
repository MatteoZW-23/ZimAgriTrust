...
import {
  ...
  getTransportNegotiation,
  rejectTransportNegotiation,
  sendTransportNegotiationMessage,
  sendTransportNegotiationOffer,
} from "../api";
...
export default function TransportNegotiationScreen({ navigation, route }) {
  ...
  const loadNegotiation = async () => {
    setLoading(true);
    try {
      const data = await getTransportNegotiation(token, negotiationId);
      const rows = (data.messages || []).map((message) => ({
        id: message.id,
        sender: message.sender || message.sender_id || 'COUNTERPARTY',
        type: message.message_type || message.type,
        content: message.content,
        structuredOffer: message.structured_offer
          ? {
              payer: message.structured_offer.payer,
              amount: Number(message.structured_offer.amount || 0),
              splitRatio: message.structured_offer.split_ratio,
            }
          : null,
        timestamp: message.created_at || new Date().toISOString(),
      }));
      setMessages(rows);
      setCurrentOffer(data.final_amount ? {
        payer: data.final_payer,
        amount: Number(data.final_amount),
        splitRatio: data.split_ratio,
      } : null);
    } catch (error) {
      Alert.alert('Error', error.message || 'Failed to load negotiation.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSendMessage = async () => {
    ...
    setSending(true);
    try {
      await sendTransportNegotiationMessage(token, negotiationId, newMessage);
      setNewMessage('');
  ...
  const handleSendOffer = async () => {
    ...
    try {
      ...
      await sendTransportNegotiationOffer(token, {
        negotiation_id: negotiationId,
        payer: offerPayer,
        amount: structuredOffer.amount,
        split_ratio: structuredOffer.splitRatio,
        message: `Offer: $${parseFloat(offerAmount).toFixed(2)} paid by ${offerPayer === 'SPLIT' ? 'split' : offerPayer}`,
      });
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            setSending(true);
            try {
              await acceptTransportNegotiation(token, negotiationId);
              Alert.alert(
                'Offer Accepted',
  ...
  const handleRejectOffer = async () => {
    setSending(true);
    try {
      await rejectTransportNegotiation(token, negotiationId, 'I cannot accept this offer');
      await loadNegotiation();
    } catch (error) {
