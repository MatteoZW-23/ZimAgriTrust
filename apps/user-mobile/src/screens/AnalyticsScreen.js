...
export default function AnalyticsScreen({ route, navigation }) {
  ...
  const fetchAnalytics = useCallback(async () => {
    try {
      ...
      setAnalytics({
        revenue: { current: 12500, previous: 10500, growth: 19.0 },
        orders: { current: 45, previous: 38, growth: 18.4 },
        listings: { current: 12, previous: 10, growth: 20.0 },
        customers: { current: 28, previous: 22, growth: 27.3 },
        performance: {
          avg_order_value: 278,
          conversion_rate: 3.2,
        ...
        category_breakdown: [
          { name: 'Maize', value: 35, color: '#FFC107' },
          { name: 'Tobacco', value: 25, color: '#4CAF50' },
          { name: 'Vegetables', value: 20, color: '#FF9800' },
          { name: 'Fruits', value: 15, color: '#E91E63' },
      ...
      setMarketTrends({
        top_crops: [
          { crop: 'Maize', price_change: 12.5, demand: 'high' },
          { crop: 'Tobacco', price_change: -5.2, demand: 'medium' },
          { crop: 'Tomatoes', price_change: 8.7, demand: 'high' },
        ],
        market_insights: [
          'Maize prices expected to rise due to increased demand',
          'Tobacco market stabilizing after recent fluctuations',
          'Vegetable prices showing strong upward trend',
        ]
      });
    } finally {
      setLoading(false);
  ...
  const handleExport = () => {
    Alert.alert('Export Analytics', 'Choose export format:', [
      ...
      { text: 'Excel Data', onPress: () => console.log('Export Excel') },
