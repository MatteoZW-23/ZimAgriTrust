...
export default function MakeOfferScreen({ navigation, route }) {
  ...
  const listingTitle = product.title || listing.product_type || listing.crop || '--';
  const listingLocation = product.loc || listing.location
    || [listing.location_district, listing.location_province].filter(Boolean).join(', ')
    || '--';
  const listingQty = product.qty || `${Number(listing.quantity || 0).toLocaleString()} ${listing.quantity_unit || 'kg'}`;
  const listingPrice = product.price || `${formatMoney(listing.price_per_unit || 0, listing.currency || 'USD')}/${listing.quantity_unit || 'kg'}`;
  const sellerName = product.sellerName || listing.seller_name || '--';
  const sellerTrust = product.rating || String(listing.seller_trust_score || 0);

  const [qty, setQty] = useState(String(Number(listing.quantity || 0)));
  const [price, setPrice] = useState(String(Number(listing.price_per_unit || 0)));
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.card}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            ...
            <Text style={styles.sellerRating}>⭐ {sellerTrust}</Text>
          </View>
          <View style={{ flexDirection: 'row', gap: 10, marginTop: 8 }}>
            <Text style={styles.sellerBadge}>✅ Verified ID</Text>
            <Text style={styles.sellerBadge}>📍 {listingLocation}</Text>
          </View>
        </View>
      </View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.card}>
          <Text style={styles.detailRow}>Product: <Text style={styles.detailVal}>{listingTitle}</Text></Text>
          <Text style={styles.detailRow}>Available: <Text style={styles.detailVal}>{listingQty}</Text></Text>
          <Text style={styles.detailRow}>Asking Price: <Text style={[styles.detailVal, { color: theme.colors.sky }]}>{listingPrice}</Text></Text>
          <Text style={styles.detailRow}>Min Order: <Text style={styles.detailVal}>--</Text></Text>
        </View>
      </View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.formCard}>
          ...
          <View style={styles.stepperInput}>
            <TouchableOpacity onPress={() => setQty(Math.max(100, parseInt(qty) - 10).toString())} style={styles.stepBtn}>
              <Text style={styles.stepBtnText}>-</Text>
            </TouchableOpacity>
            <TextInput style={styles.input} value={qty} onChangeText={setQty} keyboardType="numeric" />
            <TouchableOpacity onPress={() => setQty((parseInt(qty) + 10).toString())} style={styles.stepBtn}>
              <Text style={styles.stepBtnText}>+</Text>
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.stepperInput}>
            <TouchableOpacity onPress={() => setPrice(Math.max(0.1, parseFloat(price) - 0.01).toFixed(2))} style={styles.stepBtn}>
              <Text style={styles.stepBtnText}>-</Text>
            </TouchableOpacity>
            <TextInput style={styles.input} value={price} onChangeText={setPrice} keyboardType="numeric" />
            <TouchableOpacity onPress={() => setPrice((parseFloat(price) + 0.01).toFixed(2))} style={styles.stepBtn}>
              <Text style={styles.stepBtnText}>+</Text>
            </TouchableOpacity>
          </View>
          <Text style={styles.hint}>Asking: {listingPrice}</Text>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.card}>
          ...
          <View style={[styles.summaryRow, { borderTopWidth: 1, borderTopColor: '#EEE', paddingTop: 12, marginTop: 12 }]}>
            <Text style={[styles.summaryLabel, { fontWeight: '800', color: theme.colors.black }]}>Total to Pay:</Text>
            <Text style={[styles.summaryVal, { fontWeight: '800', color: theme.colors.green, fontSize: 18 }]}>${total.toFixed(2)}</Text>
          </View>
      ...
      <TouchableOpacity style={[styles.submitBtn, submitting && { opacity: 0.6 }]} onPress={handleSubmitOffer} disabled={submitting}>
        {submitting ? (
          <ActivityIndicator color="#fff" />
        ) : (
          <Text style={styles.submitBtnText}>SUBMIT OFFER & PAY ${total.toFixed(2)}</Text>
        )}
      </TouchableOpacity>
