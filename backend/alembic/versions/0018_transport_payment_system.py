...
TRANSPORT_REQUEST_STATUS_ENUM = postgresql.ENUM(
...
TRANSPORT_QUOTE_STATUS_ENUM = postgresql.ENUM(
    "ACTIVE", "ACCEPTED", "EXPIRED", "CANCELLED",
    name="transportquotestatus",
    create_type=False,
)

NEGOTIATION_STATUS_ENUM = postgresql.ENUM(
...
def upgrade() -> None:
    ...
    try:
        op.get_bind().execute(text("CREATE TYPE transportmode AS ENUM ('PLATFORM_DELIVERY_BUYER_REQUESTED', 'PLATFORM_DELIVERY_FARMER_REQUESTED', 'SELF_PICKUP', 'SELF_DELIVERY', 'NEGOTIATED_TRANSPORT', 'DEFERRED')"))
    except Exception:
        pass
    try:
        op.get_bind().execute(text("CREATE TYPE transportrequeststatus AS ENUM ('PENDING', 'PRICING', 'PRICED', 'ASSIGNING', 'ASSIGNED', 'IN_NEGOTIATION', 'ACCEPTED', 'REJECTED', 'CANCELLED', 'COMPLETED')"))
    except Exception:
        pass
    try:
        op.get_bind().execute(text("CREATE TYPE transportquotestatus AS ENUM ('ACTIVE', 'ACCEPTED', 'EXPIRED', 'CANCELLED')"))
    except Exception:
        pass
    try:
        op.get_bind().execute(text("CREATE TYPE negotiationstatus AS ENUM ('INITIATED', 'COUNTER_OFFER', 'ACCEPTED', 'REJECTED', 'CANCELLED', 'EXPIRED', 'ADMIN_REVIEW', 'RESOLVED')"))
    except Exception:
        pass
    try:
        op.get_bind().execute(text("CREATE TYPE negotiationmessagetype AS ENUM ('TEXT', 'OFFER', 'COUNTER_OFFER', 'ACCEPTANCE', 'REJECTION', 'SYSTEM')"))
    except Exception:
        pass
    try:
        op.get_bind().execute(text("CREATE TYPE driverassignmentstatus AS ENUM ('ASSIGNED', 'ACCEPTED', 'REJECTED', 'CANCELLED', 'COMPLETED')"))
    except Exception:
        pass
    try:
        op.get_bind().execute(text("CREATE TYPE deliverystatus AS ENUM ('PENDING', 'ASSIGNED', 'DRIVER_EN_ROUTE', 'AT_PICKUP', 'PICKED_UP', 'IN_TRANSIT', 'AT_DELIVERY', 'DELIVERED', 'CANCELLED', 'FAILED')"))
    except Exception:
        pass
    try:
        op.get_bind().execute(text("CREATE TYPE trackingstatus AS ENUM ('MOVING', 'STOPPED', 'IDLE')"))
    except Exception:
        pass
    try:
        op.get_bind().execute(text("CREATE TYPE paymentallocationtype AS ENUM ('GOODS_PAYMENT', 'TRANSPORT_FEE', 'PLATFORM_FEE', 'REFUND')"))
    except Exception:
        pass
    try:
        op.get_bind().execute(text("CREATE TYPE paymentallocationstatus AS ENUM ('PENDING', 'HELD', 'RELEASED', 'FAILED', 'REFUNDED')"))
    except Exception:
        pass
    try:
        op.get_bind().execute(text("CREATE TYPE settlementtype AS ENUM ('FARMER_PAYOUT', 'DRIVER_PAYOUT', 'BUYER_REFUND')"))
    except Exception:
        pass
    try:
        op.get_bind().execute(text("CREATE TYPE settlementstatus AS ENUM ('PENDING', 'PROCESSING', 'COMPLETED', 'FAILED')"))
    except Exception:
        pass
    try:
        op.get_bind().execute(text("CREATE TYPE disputestatus AS ENUM ('OPEN', 'UNDER_REVIEW', 'RESOLVED', 'CLOSED', 'ESCALATED')"))
    except Exception:
        pass
    try:
        op.get_bind().execute(text("CREATE TYPE notificationstatus AS ENUM ('PENDING', 'SENT', 'DELIVERED', 'FAILED', 'READ')"))
    except Exception:
        pass
    ...
    # notifications table already created in migration 0013, skip creation here
    # If needed, add missing columns via ALTER TABLE in a future migration
...
def downgrade() -> None:
    ...
    # notifications table dropped in migration 0013 downgrade
