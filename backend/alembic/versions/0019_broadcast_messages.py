...
BROADCAST_AUDIENCE_ENUM = postgresql.ENUM(
    "all","supliers" ,"farmers", "buyers", "agents", "drivers", "regional",
...
def upgrade() -> None:
    ...
    try:
        op.get_bind().execute(text("CREATE TYPE broadcastaudience AS ENUM ('all', 'farmers', 'buyers', 'agents', 'drivers', 'regional')"))
    except Exception:
        pass
    try:
        op.get_bind().execute(text("CREATE TYPE broadcaststatus AS ENUM ('pending', 'scheduled', 'sending', 'sent', 'failed', 'cancelled')"))
    except Exception:
        pass
    ...
    op.create_table(
        ...
        sa.Column("audience", BROADCAST_AUDIENCE_ENUM, server_default="all"),
        sa.Column("audience_filter", postgresql.JSON(), nullable=True),
        sa.Column("channels", postgresql.JSON(), nullable=True),
        sa.Column("status", BROADCAST_STATUS_ENUM, server_default="pending"),
