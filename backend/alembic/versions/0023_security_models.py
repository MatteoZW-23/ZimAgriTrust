...
def upgrade() -> None:
    ...
    # fraud_alerts
    fraud_severity_enum.create(op.get_bind(), checkfirst=True)
    fraud_alert_type_enum.create(op.get_bind(), checkfirst=True)
    op.create_table(
        "fraud_alerts",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("user_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.id"), nullable=True),
        sa.Column("alert_type", fraud_alert_type_enum, nullable=False),
    ...
    op.create_table(
        "withdrawal_limits",
        sa.Column("id", sa.Integer(), primary_key=True, autoincrement=True),
        sa.Column("user_tier", user_tier_enum, nullable=False, unique=True),
