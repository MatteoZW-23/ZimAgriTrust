...
class GovernanceService:
    def log_audit_event(self, db: Session, payload: dict):
        """
        PLATFORM GOVERNANCE: Logs a persistent audit event.
        Used for monitoring infrastructure integrity, role changes, and escrow overrides.
        """
        try:
            event = SystemAudit(
            ...
            db.add(event)
            db.commit()
            print(f"GOVERNANCE LOG | {payload.get('type')} | Severity: {payload.get('severity')}")
        except Exception as e:
            # Fallback to standard logging to avoid stopping the primary transaction
            print(f"FAILED TO PERSIST GOVERNANCE LOG: {str(e)}")
