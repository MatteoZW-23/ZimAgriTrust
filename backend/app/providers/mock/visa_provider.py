"""
Mock Visa Payment Provider
Simulates Visa card payment behavior
"""
...
class MockVisaProvider(BasePaymentProvider):
    """Mock Visa provider simulating real card payment behavior"""
    
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        super().__init__(config)
        self.provider_name = "Visa"
        self.api_key = self.config.get("api_key", "mock_visa_key")
        self.secret_key = self.config.get("secret_key", "mock_visa_secret")
        self.merchant_code = self.config.get("merchant_code", "VSA001")
        self._webhook_queue: list = []
        
        self.success_rate = self.config.get("success_rate", 0.98)
    ...
    async def initialize_payment(self, request: PaymentRequest) -> PaymentResponse:
        ...
        if rand < self.failure_rate:
            ...
            response = PaymentResponse(
                ...
                reference=request.reference,
                provider_reference=provider_ref,
                message="Payment timed out: Card network timeout",
                created_at=datetime.utcnow(),
    ...
    async def process_withdrawal(
        ...
        if rand < self.failure_rate:
            response = PaymentResponse(
                ...
                reference=reference,
                provider_reference=provider_ref,
                message="Withdrawal failed: Card not supported for withdrawal",
                created_at=datetime.utcnow(),
