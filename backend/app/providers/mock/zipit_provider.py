"""
Mock ZIPIT Payment Provider
Simulates ZIPIT bank transfer behavior for Zimbabwe
"""
...
class MockZIPITProvider(BasePaymentProvider):
    """Mock ZIPIT provider simulating real bank transfer behavior"""
    
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        super().__init__(config)
        self.provider_name = "ZIPIT"
        self.api_key = self.config.get("api_key", "mock_zipit_key")
        self.secret_key = self.config.get("secret_key", "mock_zipit_secret")
        self.merchant_code = self.config.get("merchant_code", "ZIP001")
        self._webhook_queue: list = []
        
        self.success_rate = self.config.get("success_rate", 0.96)
        self.delay_range = self.config.get("delay_range", (3, 8))
        self.timeout_rate = self.config.get("timeout_rate", 0.01)
    ...
    async def initialize_payment(self, request: PaymentRequest) -> PaymentResponse:
        ...
        if rand < self.failure_rate:
            response = PaymentResponse(
                ...
                message="Payment failed: Bank transfer declined",
            ...
            response = PaymentResponse(
                ...
                reference=request.reference,
                provider_reference=provider_ref,
                message="Payment timed out: Bank system unavailable",
                created_at=datetime.utcnow(),
    ...
    async def process_withdrawal(
        ...
        if rand < self.failure_rate:
            response = PaymentResponse(
                ...
                reference=reference,
                provider_reference=provider_ref,
                message="Withdrawal failed: Invalid bank account",
                created_at=datetime.utcnow(),
