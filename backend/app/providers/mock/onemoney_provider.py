"""
Mock OneMoney Payment Provider
Simulates OneMoney mobile money behavior for Zimbabwe
"""
...
class MockOneMoneyProvider(BasePaymentProvider):
    """
    Mock OneMoney provider simulating real mobile money behavior
    Supports: payments, withdrawals, refunds, webhooks
    """
    
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        super().__init__(config)
        self.provider_name = "OneMoney"
        self.api_key = self.config.get("api_key", "mock_onemoney_key")
        self.secret_key = self.config.get("secret_key", "mock_onemoney_secret")
        self.merchant_code = self.config.get("merchant_code", "ONE001")
        self._webhook_queue: list = []
        
        # Simulation parameters
        self.success_rate = self.config.get("success_rate", 0.93)
        self.delay_range = self.config.get("delay_range", (2, 6))
        self.timeout_rate = self.config.get("timeout_rate", 0.03)
    ...
    async def initialize_payment(self, request: PaymentRequest) -> PaymentResponse:
        ...
        if rand < self.failure_rate:
            response = PaymentResponse(
                ...
                message="Payment failed: Account not active or insufficient funds",
            ...
            response = PaymentResponse(
                ...
                reference=request.reference,
                provider_reference=provider_ref,
                message="Payment timed out: User did not confirm PIN",
                created_at=datetime.utcnow(),
                failure_reason="PIN_TIMEOUT",
                metadata={"provider": self.provider_name, "phone": request.phone_number}
            )
        else:
            response = PaymentResponse(
    ...
    async def verify_payment(self, transaction_id: str) -> PaymentResponse:
        """Verify payment status with OneMoney"""
    ...
    async def process_withdrawal(
        ...
        """Process withdrawal to OneMoney wallet"""
        transaction_id = self._generate_transaction_id()
        provider_ref = self._generate_provider_reference()
        ...
        if rand < self.failure_rate:
            response = PaymentResponse(
                ...
                reference=reference,
                provider_reference=provider_ref,
                message="Withdrawal failed: Account limit exceeded",
                created_at=datetime.utcnow(),
    ...
    async def handle_webhook(self, payload: Dict[str, Any]) -> PaymentResponse:
        """Process OneMoney webhook callback"""
    ...
    def generate_webhook_signature(self, payload: Dict[str, Any]) -> str:
        """Generate OneMoney webhook signature"""
