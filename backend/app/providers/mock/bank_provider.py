"""
Mock Bank Payment Provider
Simulates bank transfer behavior for Zimbabwe
"""
...
class MockBankProvider(BasePaymentProvider):
    """Mock Bank provider simulating real bank transfer behavior"""
    
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        super().__init__(config)
        self.provider_name = "Bank"
        self.api_key = self.config.get("api_key", "mock_bank_key")
        self.secret_key = self.config.get("secret_key", "mock_bank_secret")
        self.merchant_code = self.config.get("merchant_code", "BNK001")
        ...
        self.failure_rate = self.config.get("failure_rate", 0.02)
    ...
    async def initialize_payment(self, request: PaymentRequest) -> PaymentResponse:
        ...
        if rand < self.failure_rate:
            response = PaymentResponse(
                ...
                message="Payment failed: Bank transfer rejected",
            ...
            response = PaymentResponse(
                ...
                reference=request.reference,
                provider_reference=provider_ref,
                message="Payment timed out: Bank processing delayed",
                created_at=datetime.utcnow(),
                failure_reason="BANK_DELAY",
                metadata={"provider": self.provider_name, "phone": request.phone_number}
            )
        else:
            response = PaymentResponse(
    ...
    async def process_withdrawal(
        ...
        if rand < self.failure_rate:
            response = PaymentResponse(
                ...
                reference=reference,
                provider_reference=provider_ref,
                message="Withdrawal failed: Account verification failed",
                created_at=datetime.utcnow(),
                failure_reason="ACCOUNT_VERIFICATION_FAILED",
